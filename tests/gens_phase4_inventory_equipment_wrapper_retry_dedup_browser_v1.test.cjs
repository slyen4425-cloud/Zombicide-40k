...
(async()=>{
  ...
  assert.deepEqual(finalChain.open,[
    ...
    'equipment-hotfix:__equipmentHotfix167817',
    'native'
  ],'Hero Editor retry must not duplicate an existing open responsibility below Cleanup');
  ...
  assert.deepEqual(finalChain.save,[
    ...
    'equipment-hotfix:__equipmentHotfix167817',
    'native'
  ],'Hero Editor retry must not duplicate an existing save responsibility below Cleanup invalidation');
  ...
  console.log(JSON.stringify({
    scenario:'Phase 4 Equipment wrapper retry dedup browser contract',
    initial,
    finalChain,
    effects,
    finding:'Hero Editor Dynamic stays unique after retry; one save produces one saveCustomEquipment write'
  },null,2));
