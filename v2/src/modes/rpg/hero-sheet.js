...
function assetPathFromArtId(artId,kind='creature'){
  const value=String(artId||'').trim();
  if(!value) return null;
  if(/^https?:\/\//i.test(value)||value.startsWith('../')||value.startsWith('./')||value.startsWith('/')) return value;
  if(/\.(png|jpg|jpeg|webp)$/i.test(value)) return kind==='item'?`../assets/dungeon/items/${value}`:`../assets/dungeon/creatures/${value}`;
  return null;
}
function itemAsset(item={}){return assetPathFromArtId(item.artId,'item');}
