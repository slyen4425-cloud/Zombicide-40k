/* GenSrpG Tactical Combat V2 — experimental tactile battlefield UI.
...
(function(root,factory){
  ...
  function logText(row){
    ...
    if(row.type==="attack")return `${actorLabel(E().actorById(battle,row.attackerId))} → ${actorLabel(E().actorById(battle,row.targetId))} : ${row.hit?`${row.damage} dégât(s) [${row.roll}/${row.hitChance}%]`:`raté [${row.roll}/${row.hitChance}%]`}`;
  ...
  function renderGrid(){
    const grid=battle.grid,reach=reachableSet(),blocked=new Set((grid.blocked||[]).map(p=>key(p.x,p.y))),cur=current();
    ...
    for(let y=0;y<grid.height;y++)for(let x=0;x<grid.width;x++){
      ...
      if(blocked.has(k))cls.push("blocked");if(reach.has(k)&&(!a||a.id===cur?.id))cls.push("reachable");
  ...
  function renderActions(){
    ...
    const attacks=(cur.attacks||[]).map(a=>{const p=target?E().attackPreview(battle,cur.id,target.id,a.id):null;const disabled=!target||!p?.ok||cur.actionsLeft<1;const label=`${esc(a.name)} · ${a.minRange}-${a.maxRange} cases${p?.ok?` · ${p.hitChance}% · ${p.damage} dmg`:""}`;return `<button class="gtv2Btn ${a.id===selectedAttack?"good":""}" data-attack="${esc(a.id)}" ${disabled?"disabled":""}>${label}</button>`}).join("");
  ...
  function render(){
    ...
    rootEl.innerHTML=`<div class="gtv2Shell"><div class="gtv2Top"><strong>⚔️ Combat tactique V2 · manche ${battle.round}</strong><button class="gtv2Btn danger" data-close>Fermer sans appliquer</button></div>
      <div class="gtv2Hud"><div class="gtv2Card"><strong>${cur?`Tour : ${esc(actorLabel(cur))}`:"Combat terminé"}</strong><br><small>${cur?`Mouvement ${cur.movementLeft}/${cur.movement} · Action ${cur.actionsLeft}`:""}</small></div><div class="gtv2Card"><strong>${target?`Cible : ${esc(actorLabel(target))}`:"Touchez un ennemi pour le cibler"}</strong><br><small>${target?`PV ${target.hp}/${target.maxHp} · DEF ${target.defense} · ARM ${target.armor}`:"Portée et ligne de vue sont calculées en cases."}</small></div></div>
      <div class="gtv2BattleWrap">${renderGrid()}</div><div class="gtv2Legend">Vert = déplacement possible · Rouge = cible attaquable · un obstacle bloque la ligne de vue.</div>${renderActions()}
      ${battle.status==="ended"?`<div class="gtv2Winner">${battle.winner==="hero"?"🏆 Victoire":"💀 Défaite"}</div><div class="gtv2Actions"><button class="gtv2Btn good" data-commit>Appliquer le résultat et fermer</button></div>`:""}
  ...
  function close(apply=false){
    if(apply&&battle)try{A()?.commitBattle?.(R,battle,{removeDefeatedEnemies:true})}catch(e){console.error("tactical commit",e)}
    rootEl?.remove();rootEl=null;const b=battle;battle=null;selectedTarget="";selectedAttack="";return b;
  }
  function onClick(ev){
