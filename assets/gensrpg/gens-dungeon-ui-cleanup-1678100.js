/* GenSrpG V16.78.107 — targeted Dungeon UI cleanup.
   Built-in party-card art is repaired against the real late internal renderer with a host-scoped observer only. */
(function(){
"use strict";
const R=typeof window!=="undefined"?window:globalThis,D=typeof document!=="undefined"?document:null;
const VERSION="1.2.0",APP_VERSION="16.78.107";
...
const PERCEPTION_SRC="assets/gensrpg/gens-canonical-perception-1678107.js?v=1678107";
...
function observePartyHost(){if(!D||typeof MutationObserver!=="function")return false;const host=D.getElementById("dc01Heroes");if(!host)return false;if(partyHost===host&&partyObserver)return true;try{partyObserver?.disconnect?.()}catch(e){}partyHost=host;partyObserver=new MutationObserver(()=>{if(repairingParty)return;repairPartyCards()});partyObserver.observe(host,{childList:true,subtree:true,attributes:true,attributeFilter:["src","style","hidden"]});repairPartyCards();return true}
...
function loadPerception(){if(!D||R.GensCanonicalPerception1678107||D.querySelector('script[data-gcp1678107]'))return false;const s=D.createElement("script");s.src=PERCEPTION_SRC;s.async=false;s.setAttribute("data-gcp1678107","1");(D.body||D.head||D.documentElement).appendChild(s);return true}
function run(){loadPerception();observePartyHost();repairPartyCards();hideLegacyHeroInputs();cleanRuleSummary();return true}
...
function install(){if(installed)return true;installed=true;loadPerception();observePartyHost();hookAll();schedule();let tries=0;const retry=()=>{hookAll();loadPerception();observePartyHost();schedule();if(tries++<60)setTimeout(retry,100)};setTimeout(retry,50);return true}
R.GensDungeonUiCleanup1678100={VERSION,APP_VERSION,ART,LEGACY_IDS,PERCEPTION_SRC,repairPartyCards,observePartyHost,hideLegacyHeroInputs,cleanRuleSummary,loadPerception,run,install};
