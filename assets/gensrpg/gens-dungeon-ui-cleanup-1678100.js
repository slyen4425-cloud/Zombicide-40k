/* GenSrpG V16.78.106 — targeted Dungeon UI cleanup.
...
function install(){if(installed)return true;installed=true;observePartyHost();hookAll();schedule();let tries=0;const retry=()=>{hookAll();observePartyHost();schedule();if(tries++<60)setTimeout(retry,100)};setTimeout(retry,50);return true}
R.GensDungeonUiCleanup1678100={VERSION,APP_VERSION,ART,LEGACY_IDS,repairPartyCards,observePartyHost,hideLegacyHeroInputs,cleanRuleSummary,run,install};
