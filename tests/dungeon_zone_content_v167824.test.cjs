...
const legacyLinked=api.normalizeContent({mode:'fixed',templateLinked:true,templateRoomId:'room1',chests:[{id:'legacy',cell:5,rarity:'common',gold:0,items:[]}]});
assert.equal(legacyLinked.templateLinked,true,'legacy automatic template-link marker must survive normalization until migration');
assert.equal(legacyLinked.templateRoomId,'room1','legacy automatic template room id must survive normalization until migration');
