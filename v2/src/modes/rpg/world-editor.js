...
export function mountWorldEditor(host){
  ...
  function render(){
    ...
    host.innerHTML=`<section class="workspace-head"><div><p class="eyebrow">RPG · WORLD BUILDER</p><h2>${esc(draft.world.name)}</h2><p class="muted">Crée les zones, salles et passages. Les liaisons se font uniquement par menus : aucun ID technique à saisir.</p></div><button class="help-button" type="button" data-help="rpg-world-builder">?</button></section>
      ...
      <section class="editor-section"><div class="section-title-row"><div><h3>Liaisons</h3><p class="muted">Choisis la salle de départ et la destination. Un passage peut être bidirectionnel ou à sens unique.</p></div><button class="primary-button" id="addWorldLink" type="button" ${draft.rooms.length>1?'':'disabled'}>+ Liaison</button></div><div class="editor-list">${draft.links.map(l=>linkCard(l,draft.rooms)).join('')||'<p class="muted">Aucune liaison.</p>'}</div></section>`;
