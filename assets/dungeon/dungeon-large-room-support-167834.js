...
function syncEntranceState(){const x=readRt();if(!DOC||!x)return false;DOC.body?.classList?.toggle("dc054AtEntrance",Number(x.room||0)<=0);return true}
function wrapRender(){const core=ROOT.DungeonCore01;if(!core)return false;for(const name of ["render","show"]){const old=core[name];if(typeof old!=="function"||old.__dlr167835)continue;const w=function(){const r=old.apply(this,arguments);syncEntranceState();patchLabels();retireDuplicateWorldActivation();markLarge();return r};w.__dlr167835=true;w.__dlr167835Original=old;core[name]=w}return true}
...
ROOT.DungeonLargeRoom167834={VERSION,APP_VERSION,MIN_SIZE,MAX_SIZE,PRIMARY_KEY,gridSize,generate,readPrimarySelection,selectedWorld,repairGenerated,repairWorld,roomMap,retireDuplicateWorldActivation,syncEntranceState,install};
