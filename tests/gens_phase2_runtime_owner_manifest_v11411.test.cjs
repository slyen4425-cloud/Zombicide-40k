...
assert.equal(reachable.size,75,'production graph size drifted before owner validation');
assert.equal(mapped.size,75,'owner manifest must currently map all 75 production-reachable files');
