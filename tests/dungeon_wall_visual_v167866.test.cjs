...
assert.match(runtime,/APP_VERSION="16\.78\.68"/);
...
assert.match(sw,/gensrpg-cache-16\.78\.68-runtime-visual-load/);
assert.match(sw,/dungeon-authored-cache-visual-167852\.js/);
assert.match(sw,/assets\/dungeon\/creatures\/dng_wall_block\.jpg/);

if(siteIndex){
  assert.match(siteIndex,/dungeon-authored-cache-visual-167852\.js\?v=167868/,'runtime visual bridge must be injected in deployed index');
}

console.log('Dungeon wall/runtime visual V16.78.68 regression: OK');
