...
(function(root,factory){
  ...
  "use strict";
  const VERSION="1.0.0",APP_VERSION="16.78.114.13";
  ...
  function patchOpenHero(){
    ...
    const wrapped=function(id){
      ...
      try{const sheet=D()?.getElementById?.("sheet");if(allowedHero(id)&&!visible(sheet))forceOpenHero(id)}catch(e){}
      return out;
    };
  ...
  function bindHeroCards(){
    ...
    doc.addEventListener("click",ev=>{
      ...
      const core=R.DungeonCore01;if(typeof core?.openHero==="function")core.openHero(id);else forceOpenHero(id);
    },true);
  ...
  function installWithRetries(){
    for(const ms of RETRIES){if(ms===0)install();else if(typeof R.setTimeout==="function")R.setTimeout(install,ms)}return true;
  }
  const api={VERSION,APP_VERSION,WALL_ASSET,STYLE_ID,RETRIES,runtimeState,unwrapMarked,restoreNativeMapRenderer,restoreDungeonRenderers,cleanWallCells,resetWalls,participants,cardHeroId,allowedHero,forceOpenHero,patchOpenHero,bindHeroCards,forceRootHome,patchQuit,install,installWithRetries,status:()=>({installed,cardBound,quitPatched,heroPatched,resetCount})};
