...
const VERSION="1.0.0",APP_VERSION="16.78.99",SCHEMA_VERSION=1;
...
function migrateAll(){const all=profiles();let changed=false;for(const p of all){if(p?.gameStyle!=="dungeon")continue;const before=JSON.stringify(p.rpgUniverse?.stats||{});ensure(p,false);if(before!==JSON.stringify(p.rpgUniverse?.stats||{}))changed=true}if(changed)saveProfiles(all);return changed}
function definitions(opt={}){const s=ensure(profile(),false);let list=(s?.definitions||[]).map(clone);if(opt.activeOnly){const a=new Set((s?.active||[]).map(canon));list=list.filter(d=>a.has(d.id))}if(opt.visibleOnly)list=list.filter(d=>d.visible!==false);return list}
function definition(id){id=canon(id);return definitions().find(d=>d.id===id)||null}
function activeIds(){return new Set((ensure(profile(),false)?.active||[]).map(canon))}
function isActive(id){return activeIds().has(canon(id))}
function persistDefinitions(list,active=null){const p=profile();if(!p)return false;const s=ensure(p,false),out=[],seen=new Set();for(const raw of list||[]){const d=normalizeDef(raw);if(d&&!seen.has(d.id)){out.push(d);seen.add(d.id)}}for(const c of CORE)if(!seen.has(c.id)){out.push(clone(c));seen.add(c.id)}s.definitions=out;s.customStats=clone(out);if(active)s.active=[...new Set([...active].map(canon).filter(Boolean))];return ensure(p,true)&&true}
function setActive(id,on){id=canon(id);const p=profile(),s=ensure(p,false);if(!s||!id)return false;const a=new Set(s.active.map(canon));on?a.add(id):a.delete(id);s.active=[...a];ensure(p,true);renderSheet();return true}
...
function assignedValue(hero,id,def=definition(id)){id=canon(id);const st=heroState(hero);let v=Number(st?.rpgAttributes?.[id]);if(Number.isFinite(v))return v;v=Number(st?.customStats?.[id]);if(Number.isFinite(v))return v;return heroDefinitionBase(hero,id,def)}
...
function temporaryBonus(id){try{return Number(R.dungeonChallengeDebuffTotal067?.(canon(id),R.state))||0}catch(e){return 0}}
function effectiveValue(hero,id){id=canon(id);const def=definition(id);if(!def)return 0;let v=assignedValue(hero,id,def);if(String(hero)===String(R.current||"")){v+=equipmentBonus(id)+talentBonus(id)+temporaryBonus(id)}try{const api=R.GensCustomStats167879;if(api?.applyEffects)v=Number(api.applyEffects(v,hero,"attribute:"+id,{min:def.min,max:def.max}))}catch(e){}return clamp(Number(v)||0,def.min,def.max)}
function saveHero(hero,st){try{if(String(hero)===String(R.current||"")&&R.state===st){R.save?.();return true}if(typeof R.key==="function"){localStorage.setItem(R.key(hero),JSON.stringify(st));return true}if(typeof R.saveState==="function"){R.saveState(hero,st);return true}}catch(e){}return false}
function syncState(hero,st=heroState(hero)){if(!st)return null;st.rpgAttributes=st.rpgAttributes&&typeof st.rpgAttributes==="object"?st.rpgAttributes:{};st.customStats=st.customStats&&typeof st.customStats==="object"?st.customStats:{};for(const d of definitions()){if(!Number.isFinite(Number(st.rpgAttributes[d.id]))){const v=assignedValue(hero,d.id,d);st.rpgAttributes[d.id]=v}st.customStats[d.id]=Number(st.rpgAttributes[d.id])||0}return st}
function change(hero,id,delta){
 hero=String(hero||R.current||"");id=canon(id);delta=Number(delta)||0;if(!hero||!delta)return false;const def=definition(id),st=syncState(hero);if(!def||!st||def.editMode==="runtime")return false;
...
function setValue(hero,id,value,opt={}){id=canon(id);const cur=assignedValue(hero,id,definition(id));if(opt.free){const d=definition(id),st=syncState(hero);if(!d||!st)return false;const n=clamp(Number(value)||0,d.min,d.max);st.rpgAttributes[id]=n;st.customStats[id]=n;saveHero(hero,st);renderSheet();return true}return change(hero,id,(Number(value)||0)-cur)}
function bonusFields(){const out=SYSTEM_BONUSES.map(clone),seen=new Set(out.map(x=>x.key));for(const d of definitions()){if(!seen.has(d.id)){out.push({key:d.id,label:d.name});seen.add(d.id)}}return out}
function description(id){const d=definition(id);if(!d)return "";const own=String(d.description||"").trim();let links="";try{links=String(R.GensGenericStats167887?.descriptionForSource?.(d.id)||"").trim()}catch(e){}return {text:own,links:links&&links!=="Aucune liaison automatique."?links:""}}
function renderEditor(){
 if(!D)return false;const host=D.getElementById("rpgStatsList");if(!host)return false;const s=ensure(profile(),false);if(!s)return false;D.getElementById("gcs167879Box")?.remove?.();
 const active=new Set(s.active.map(canon));host.dataset.gensStatService="167899";
 host.innerHTML='<div class="gss899Toolbar"><button type="button" id="gss899Add">＋ AJOUTER UNE STAT</button><small>Une seule liste : ces mêmes statistiques sont utilisées en jeu, par les talents, équipements et sets.</small></div>'+s.definitions.map((d,i)=>'<details class="gss899Stat" data-stat-id="'+attr(d.id)+'"><summary><label onclick="event.stopPropagation()"><input type="checkbox" data-gss-active="1" '+(active.has(d.id)?'checked':'')+'> '+esc((d.icon?d.icon+' ':'')+d.name)+'</label><span>'+esc(d.id)+'</span></summary><div class="gss899Grid"><label>Nom<input data-gss="name" value="'+attr(d.name)+'"></label><label>Icône<input data-gss="icon" value="'+attr(d.icon)+'"></label><label>Minimum<input type="number" data-gss="min" value="'+d.min+'"></label><label>Maximum<input type="number" data-gss="max" value="'+d.max+'"></label><label>Valeur de départ<input type="number" data-gss="defaultValue" value="'+d.defaultValue+'"></label><label>Modification<select data-gss="editMode"><option value="points"'+(d.editMode==="points"?' selected':'')+'>Points</option><option value="free"'+(d.editMode==="free"?' selected':'')+'>Libre</option><option value="runtime"'+(d.editMode==="runtime"?' selected':'')+'>Jeu seulement</option></select></label></div><label>Description<textarea data-gss="description">'+esc(d.description||"")+'</textarea></label>'+(CORE.some(c=>c.id===d.id)?'':'<button type="button" data-gss-remove="1">🗑️ SUPPRIMER</button>')+'</details>').join("");
...
function editorSnapshot(){const host=D?.getElementById("rpgStatsList");if(!host)return null;const defs=definitions();const byId=new Map(defs.map(d=>[d.id,d]));const out=[],active=[];for(const row of host.querySelectorAll?.(".gss899Stat")||[]){const id=canon(row.dataset.statId),old=byId.get(id);if(!old)continue;const q=f=>row.querySelector?.('[data-gss="'+f+'"]');out.push(normalizeDef({...old,name:q("name")?.value,icon:q("icon")?.value,min:q("min")?.value,max:q("max")?.value,defaultValue:q("defaultValue")?.value,editMode:q("editMode")?.value,description:q("description")?.value}));if(row.querySelector?.('[data-gss-active="1"]')?.checked)active.push(id)}return {defs:out,active}}
function saveEditor(){const snap=editorSnapshot();if(!snap)return false;const ok=persistDefinitions(snap.defs,snap.active);try{R.GensGenericStats167887?.renderLinksEditor?.()}catch(e){}renderSheet();return ok}
function addDefinition(){const defs=definitions(),ids=new Set(defs.map(d=>d.id));let n=1,id="nouvelle_stat";while(ids.has(id))id="nouvelle_stat_"+(++n);defs.push({id,name:"Nouvelle stat",icon:"📊",kind:"score",min:0,max:30,defaultValue:0,visible:true,editMode:"points",description:"",effects:[]});const active=[...activeIds(),id];persistDefinitions(defs,active);renderEditor();}
function removeDefinition(id){id=canon(id);if(CORE.some(c=>c.id===id))return false;const defs=definitions().filter(d=>d.id!==id),active=[...activeIds()].filter(x=>x!==id);persistDefinitions(defs,active);renderEditor();return true}
function bindEditor(){const host=D?.getElementById("rpgStatsList");if(!host||host.dataset.gss899Bound)return;host.dataset.gss899Bound="1";host.addEventListener("change",()=>saveEditor());host.addEventListener("click",e=>{if(e.target?.id==="gss899Add"){e.preventDefault();addDefinition()}const b=e.target?.closest?.('[data-gss-remove="1"]');if(b){e.preventDefault();const row=b.closest?.(".gss899Stat");if(row&&confirm("Supprimer cette statistique ?"))removeDefinition(row.dataset.statId)}})}
function renderSheet(){
 if(!D)return false;const host=D.getElementById("dungeonAttributeGrid");if(!host||!R.current||!R.state)return false;try{R.dungeonSyncProgressionForState?.(R.current,R.state)}catch(e){}
...
function sheetClickOnce(e){const b=e.target?.closest?.("[data-gss-change]");if(!b)return;change(R.current,b.dataset.stat,Number(b.dataset.gssChange)||0)}
...
function installRuntime(){
 if(typeof R.renderDungeonAttributes==="function"&&!R.renderDungeonAttributes.__gss899){nativeRenderAttributes=R.renderDungeonAttributes;const w=function(){let out;try{out=nativeRenderAttributes.apply(this,arguments)}catch(e){console.warn("GenSrpG stat service native summary",e)}renderSheet();return out};w.__gss899=true;R.renderDungeonAttributes=w}
 if(typeof R.renderDungeonHeroStats==="function"&&!R.renderDungeonHeroStats.__gss899){nativeRenderHeroStats=R.renderDungeonHeroStats;const w=function(){renderSheet();const compact=D?.getElementById("dungeonHeroStats");if(compact){compact.style.display="none";compact.innerHTML=""}return true};w.__gss899=true;R.renderDungeonHeroStats=w}
 if(typeof R.dungeonAttributeValue==="function"&&!R.dungeonAttributeValue.__gss899){nativeAttributeValue=R.dungeonAttributeValue;const w=function(id){const d=definition(id);return d?effectiveValue(String(R.current||""),id):nativeAttributeValue.apply(this,arguments)};w.__gss899=true;R.dungeonAttributeValue=w}
 if(typeof R.changeDungeonAttribute==="function"&&!R.changeDungeonAttribute.__gss899){nativeChange=R.changeDungeonAttribute;const w=function(id,delta){return definition(id)?change(String(R.current||""),id,delta):nativeChange.apply(this,arguments)};w.__gss899=true;R.changeDungeonAttribute=w}
 if(typeof R.renderRpgUniverseEditor==="function"&&!R.renderRpgUniverseEditor.__gss899){nativeEditor=R.renderRpgUniverseEditor;const w=function(){const out=nativeEditor.apply(this,arguments);renderEditor();return out};w.__gss899=true;R.renderRpgUniverseEditor=w}
 if(typeof R.saveRpgUniverseStats==="function"&&!R.saveRpgUniverseStats.__gss899){nativeSaveStats=R.saveRpgUniverseStats;const w=function(){saveEditor();return nativeSaveStats.apply(this,arguments)};w.__gss899=true;R.saveRpgUniverseStats=w}
 return true;
}
function patchCompatibility(){const api=R.GensCustomStats167879;if(api){api.defs=()=>definitions();api.def=id=>definition(id);api.activeIds=()=>activeIds();api.isActive=id=>isActive(id);api.value=(hero,id)=>assignedValue(hero,id,definition(id));api.setValue=(hero,id,v,opt)=>setValue(hero,id,v,opt);api.change=(id,delta)=>change(String(R.current||""),id,delta);api.renderEditor=()=>true;api.syncPrimaryList=()=>true;api.renderHero=()=>true;api.add=addDefinition;api.remove=i=>{const d=definitions()[Number(i)];return d?removeDefinition(d.id):false};api.persist=saveEditor}}
function install(){migrateAll();installRuntime();patchCompatibility();renderEditor();try{nativeRenderAttributes?.();renderSheet()}catch(e){}try{R.GENSRPG_VERSION=APP_VERSION}catch(e){}return true}
R.GensStatService167899={VERSION,APP_VERSION,SCHEMA_VERSION,ALIASES,CORE,canon,profile,ensure,migrateAll,definitions,definition,activeIds,isActive,setActive,persistDefinitions,heroState,syncState,assignedValue,effectiveValue,change,setValue,bonusFields,description,renderEditor,saveEditor,addDefinition,removeDefinition,renderSheet,installRuntime,patchCompatibility,install};
