...
(async()=>{
  ...
  try{
    ...
    console.log(JSON.stringify({scenario:"V114.10 real Chromium mobile",viewport:"412x915 @2.625x touch",engagements:6,maxOpenMs:Number(Math.max(...openings).toFixed(1)),participants:"Lyra excluded in room 0, included after entry",weapons:"sword 70%, bow 80%, origins visible",walls:`7 zooms, max near-white ${(Math.max(...ratios)*100).toFixed(1)}%`,dice:"one overlay and one animation row"}));
