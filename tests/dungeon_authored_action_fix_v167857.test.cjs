...
const legacy=control('🔍 FOUILLER LE COFFRE'),generic=control('🔎 FOUILLER'),exact=control('🎁 Ouvrir coffre legendary'),other=control('✅ FIN DU TOUR');generic.getAttribute=()=>"searchItem()";
...
assert.equal(api.exactChestAtActivePosition(),false);api.syncLegacyChestButton();assert.equal(generic.hidden,false,'generic search must return away from an exact chest');const beforeRenders=renders;context.DungeonSpatial313.persist(JSON.parse(store[RT]));while(timers.length)timers.shift().fn();assert.equal(renders,beforeRenders,'same persisted position must not trigger another movement render');
