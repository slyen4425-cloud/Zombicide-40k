...
(async()=>{
  ...
  try{
    ...
    assert.equal(first.stubScripts,20,'all 20 Pages runtime script slots must execute before preview-ready');
    ...
    assert.equal(second.stubScripts,20,'runtime composition must execute exactly once after restart');
