...
(async()=>{
  ...
  try{
    ...
    assert.equal(first.stubScripts,27,'all 27 Pages runtime script slots must execute before preview-ready');
    ...
    assert.equal(second.stubScripts,27,'runtime composition must execute exactly once after restart');
