...
const runtime={participants:['hero'],index:0,room:1,positions:{hero:0},last:{authoredRuntime167839:true,worldRuntime167823:true,worldDungeonId:'world-1',worldNodeId:'zone-1',map:{cells:['entry','floor','trap','floor','trap','floor','trap','floor','exit']},worldZoneContent167824:{mode:'fixed',traps:[trap]}}};
...
let scene=[
 ...
 {id:'other-room',kind:'trap',room:2,cellIndex:2,trapId:'dart',detected:true}
],resolved=[];
...
let exact=scene.find(e=>e.exactTrap167845);assert.ok(exact,'un piège exact de scène doit être créé');assert.equal(exact.trapId,'rune');assert.equal(exact.detected,false,'le piège exact doit rester caché par défaut');assert.equal(scene.some(e=>e.id==='manual'),false,'aucun piège de scène non authored ne doit survivre dans la salle authored courante');
assert.equal(scene.some(e=>e.id==='legacy-random'),false,'un piège aléatoire legacy dc202 doit être supprimé dans une salle construite');
assert.equal(scene.some(e=>e.id==='other-room'),true,'le nettoyage authored ne doit pas toucher aux pièges d’une autre salle');
