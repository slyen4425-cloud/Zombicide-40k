const CACHE_NAME = "gensrpg-cache-16.78.34-large-rooms-authoritative-world";
...
self.addEventListener("install", event => {
  ...
  event.waitUntil(caches.open(CACHE_NAME).then(async cache => {
    for (const file of CORE_FILES) {
      ...
      catch (error) { console.warn("Impossible de mettre en cache :", file); }
    }
  }));
});
...
self.addEventListener("fetch", event => {
  ...
  if (request.mode === "navigate" || request.destination === "document") {
    event.respondWith((async () => {
      try {
        ...
        if (freshResponse && freshResponse.ok) { const cache = await caches.open(CACHE_NAME); await cache.put(request,freshResponse.clone()); }
        return freshResponse;
      } catch (error) {
        const cached = await caches.match(request); if (cached) return cached;
        const fallback = await caches.match("./index.html"); if (fallback) return fallback;
  ...
  if (request.destination === "audio" || /\.(mp3|wav|ogg|m4a)$/i.test(url.pathname)) {
    event.respondWith((async () => {
      try {
        const fresh = await fetch(request,{cache:"no-store"});
        if (fresh && fresh.ok) { const cache = await caches.open(CACHE_NAME); await cache.put(request,fresh.clone()); }
        return fresh;
      } catch (error) { const cached = await caches.match(request); return cached || Response.error(); }
    })());
    return;
  }

  if (url.pathname.includes("/assets/dungeon/")) {
    event.respondWith((async () => {
      try {
        const fresh = await fetch(request,{cache:"no-store"});
        if (fresh && fresh.ok) { const cache = await caches.open(CACHE_NAME); await cache.put(request,fresh.clone()); }
