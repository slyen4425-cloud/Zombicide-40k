...
const HELP = {
  ...
  status: { title: 'État de la reconstruction', html: '<p>Le socle RPG sait créer des statistiques, ressources, conditions, effets, compétences et transformations avec des liens par menus.</p><p>Le combat, les déplacements, la vision, la furtivité, le World Builder et le créateur de salle utilisent maintenant des briques séparées.</p>' },
  survival: { title: 'Mode Survie', html: '<p>Le mode Survie garde ses propres règles : vagues, menace, scénarios, ennemis et progression type Zombicide. Il possède son propre espace de données et sa propre interface.</p><p><strong>Le World Builder Dungeon, le D100 RPG et les transformations RPG ne sont pas utilisés ici.</strong></p>' },
  ...
  'rpg-spatial': { title: 'Déplacements, portée & détection', html: '<p>Cette zone choisit l’unité de déplacement, la statistique éventuelle de mouvement, la portée d’entraide en combat, ainsi que les statistiques utilisées pour la vision et la furtivité.</p><p>Exemple : Mouvement = 4 cases, entraide = 3 cases, Vision = Perception, Furtivité = Discrétion. Aucun de ces noms n’est imposé au moteur.</p><p>La distance est calculée par chemin praticable : un mur oblige à contourner.</p>' },
  'rpg-world-builder': { title: 'World Builder', html: '<p>Le World Builder organise un monde en zones et salles reliées librement.</p><p>Tu peux créer des branches, caches, retours ou passages à sens unique sans imposer une suite Salle 1 → Salle 2 → Salle 3.</p><p>Les destinations sont choisies dans des menus ; aucun ID technique n’est saisi à la main.</p>' },
  'rpg-room-builder': { title: 'Créateur de salle', html: '<p>Choisis une salle créée dans le World Builder, définis ses dimensions puis peins sa grille.</p><p>Les outils placent sol, eau, lave, rocher, case bloquée, mur, porte, entrée et sortie.</p><p>La section Interactions permet maintenant de créer coffre, piège, énigme, événement, interrupteur, portail ou objet interactif, puis de le rattacher par menu à une case, une porte ou un coffre.</p><p>Les murs, portes et interactions restent des objets structurés afin que les moteurs de déplacement, vision et runtime puissent les exploiter sans logique cachée.</p>' },
...
function disposeWorkspace(){
  if(workspaceController?.dispose) workspaceController.dispose();
