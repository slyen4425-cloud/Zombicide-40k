...
for(const required of ['ensureStyle(rt)','ensureDetectionHooks(rt)','bindBoardClicks(rt)','paintWalls(rt)','animateDiceOverlay(rt)'])assert.ok(install.includes(required),`observer retirement must preserve V113 behavior: missing ${required}`);
...
console.log('GenSrpG V114.11: V113 observer retired and direct install preserved');
