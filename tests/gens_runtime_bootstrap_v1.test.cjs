...
assert.deepEqual(loaded,expected,'RuntimeBootstrap must install an available progression owner first, then preserve Tactical/Survival order');
assert.equal(sandbox.__gensTacticalV2Loader105,true,'historical loader guard must remain active');
assert.equal(progressionInstalls,1,'progression prerequisite must install once before Tactical and must not participate in delayed retries');
...
assert.deepEqual(timers,[250,1200,3000],'only historical Survival/Tactical retry timings must remain');
assert.ok(sandbox.GensRuntimeBootstrapV1,'RuntimeBootstrap API missing');
assert.equal(sandbox.GensRuntimeBootstrapV1.PROGRESSION_SRC,'assets/gensrpg/dungeon/progression-runtime-v1.js');
assert.deepEqual(Array.from(sandbox.GensRuntimeBootstrapV1.files),expected.map(x=>x.replace('?v=16.78.105','')),'RuntimeBootstrap API must expose only the delayed Tactical/Survival composition');
...
assert.equal(progressionInstalls,1,'idempotent install must not reinstall progression');

console.log('GenSrpG RuntimeBootstrap V1 progression prerequisite + Tactical/Survival load order and idempotence OK');
