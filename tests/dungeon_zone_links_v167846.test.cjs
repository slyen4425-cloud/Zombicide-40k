...
const rooms={ra:{id:'ra',name:'Salle A',cells:[{object:'entry'},{},{object:'cache'},{},{object:'exit'}]},rb:{id:'rb',name:'Salle B',cells:[{}, {object:'entry'},{},{},{object:'exit'}]},attic:{id:'attic',name:'Vieux grenier',cells:[{},{},{},{object:'entry'},{object:'exit'}]}};
...
const builder={findDungeon(id){return id==='world-1'?JSON.parse(JSON.stringify(graph)):null},loadLibrary(){return [JSON.parse(JSON.stringify(graph))]},upsertDungeon(g){Object.assign(graph,JSON.parse(JSON.stringify(g)));return g},validation(){return {valid:true,errors:[],warnings:['Sous-pièce requise absente : Vieux grenier','Cache non affectée à une instance de sous-pièce.'],cacheCount:0}}};
