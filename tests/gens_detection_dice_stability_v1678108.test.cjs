...
assert.match(src,/APP_VERSION="16\.78\.110"/);
assert.match(src,/D6_MS=520/);assert.match(src,/D100_MS=500/);assert.match(src,/WATCHDOG_MS=850/);
...
assert.match(bridge,/gens-dice-performance-1678108\.js\?v=1678110/);
...
assert.match(sw,/gensrpg-cache-16\.78\.110-combat-runtime-performance/);
...
let d6done=0;api.animateDiceFast('d6',2,[6,1],4,()=>d6done++);assert.equal(animationCalls,2,'each visible D6 must start a compositor animation');runTimers(850);assert.equal(d6done,1,'D6 callback must finish below one second');
...
now=0;timers.length=0;let d100done=0;api.animateRpgDiceFast('d100',1,[72],60,100,()=>d100done++);assert.equal(animationCalls,3,'D100 must also visibly animate');runTimers(850);assert.equal(d100done,1,'D100 callback must finish below one second');
assert.equal(boxes.d100.children.length,1);assert.match(boxes.d100.children[0].className,/success/);
console.log('V16.78.110 dice performance: visible numeric animation + bounded callbacks + no combat-hot repair scans OK');
