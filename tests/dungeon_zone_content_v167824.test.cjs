...
store.setItem(api.CONTENT_KEY,JSON.stringify({legacyWorld:{legacyNode:{mode:'fixed',templateLinked:true,templateRoomId:'room1',enemies:[],chests:[{id:'legacy',cell:5,rarity:'common',gold:0,items:[]}],traps:[],puzzles:[],npcs:[],items:[]},manualNode:{mode:'fixed',templateLinked:false,templateRoomId:'room1',enemies:[],chests:[{id:'manual',cell:5,rarity:'rare',gold:9,items:[]}],traps:[],puzzles:[],npcs:[],items:[]}}}));
assert.equal(api.legacyTemplateLinkState('legacyWorld','legacyNode','room1'),'linked','V44 automatic template copy must be identifiable from persisted provenance');
assert.equal(api.migrateLegacyTemplateLink('legacyWorld','legacyNode','room1'),true,'V44 automatic template copy must migrate once to inheritance');
assert.equal(api.getZoneContent('legacyWorld','legacyNode').mode,'inherit');
assert.equal(api.rawZoneContent('legacyWorld','legacyNode').templateLinked,undefined,'migration must consume the obsolete template link marker');
assert.equal(api.legacyTemplateLinkState('legacyWorld','manualNode','room1'),'detached','V44 explicit zone override marker must remain distinguishable');
assert.equal(api.migrateLegacyTemplateLink('legacyWorld','manualNode','room1'),false,'explicit detached zone variant must never be migrated');
assert.equal(api.getZoneContent('legacyWorld','manualNode').chests[0].rarity,'rare');
const configFlags=api.normalizeContent({mode:'fixed',enemies:[{enemyId:'dng_skeleton',configured:true}],chests:[{cell:1,configured:true}],traps:[{cell:2,configured:true}],puzzles:[{cell:3,configured:true}]});
assert.equal(configFlags.enemies[0].configured,true);assert.equal(configFlags.chests[0].configured,true);assert.equal(configFlags.traps[0].configured,true);assert.equal(configFlags.puzzles[0].configured,true);
const defaultFlags=api.normalizeContent({mode:'fixed',chests:[{cell:1}]});assert.equal(defaultFlags.chests[0].configured,false,'legacy/default specs must not masquerade as explicitly configured');
...
assert.equal(heroState.inventory.filter(i=>i.itemId==='ditem_ash_blade').length,1,'exact chest item names must resolve to the real catalog id before inventory insertion');
