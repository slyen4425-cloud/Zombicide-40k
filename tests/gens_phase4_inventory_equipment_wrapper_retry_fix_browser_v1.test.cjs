...
(async()=>{
  ...
  await page.evaluate(()=>{
    // A genuinely late function must still be picked up by the retry.
    window.openDungeonTalentNodeEditor=function(){return 'late-native'};
  });
  ...
  const finalChain=await page.evaluate(()=>({
    open:window.__describeWrapperChain('openEquipmentEditor'),
    save:window.__describeWrapperChain('saveEquipmentEditor'),
    lateTalentWrapped:!!window.openDungeonTalentNodeEditor?.__canon101
  }));
  ...
  assert.deepEqual(finalChain.open,[
    ...
    'equipment-hotfix:__equipmentHotfix167817',
    'native'
  ],'Hero Editor retry must not add a second open wrapper when its owner already exists in __original');
  ...
  assert.deepEqual(finalChain.save,[
    ...
    'equipment-hotfix:__equipmentHotfix167817',
    'native'
  ],'Hero Editor retry must not add a second save wrapper when its owner already exists in __original');
  ...
  assert.equal(finalChain.lateTalentWrapped,true,'retry must still connect a genuinely late-defined Hero Editor function');
  ...
  assert.equal(effects.saveCustomEquipmentCalls,1,
    'one user save must trigger exactly one Hero Editor Dynamic persistence write');
  ...
  console.log(JSON.stringify({
    scenario:'Phase 4 Equipment wrapper retry browser fix',
    initial,
    finalChain,
    effects,
    finding:'Hero Editor remains single-owner through Cleanup while late-defined globals are still connected'
  },null,2));
