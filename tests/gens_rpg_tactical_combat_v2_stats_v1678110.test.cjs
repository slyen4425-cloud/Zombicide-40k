...
function runtime(){
  ...
  const rt={current:'',state:null,CHARS:{hero:{name:'Hero',dungeonStats:{...values}}},loadState:()=>state,
    GensCleanRpgStats167874:{runtimeDefs:()=>defs,value:(id,stat)=>{valueCalls++;return values[stat]},extraTotal:()=>0},
    effectiveMaxWounds:()=>20,dungeonHeroMoveValue083:()=>4,dungeonDerivedInitiative:()=>13,dungeonDerivedDefense:()=>6,dungeonArmorScore:()=>2,
    dungeonDodgeChance:()=>11,dungeonCriticalChance:()=>18,dungeonMagicResistance:()=>3,dungeonMaxMana:()=>10,dungeonPhysicalDamageBonus:()=>2,dungeonMagicDamageBonus:()=>3,
    loadDungeonRpgRules:()=>({critMultiplier:2})};
  return {rt,state,getValueCalls:()=>valueCalls};
}

{
  const {rt,getValueCalls}=runtime();P.resetMetrics();
  ...
  assert.equal(P.metrics.snapshotsBuilt,1);assert.equal(P.metrics.canonicalReads,snap.canonical.length);assert.equal(getValueCalls(),snap.canonical.length);
}

{
  ...
  P.decorateBattle(rt,battle,{force:true});const first=getValueCalls();
  assert.ok(first>0);assert.equal(battle.actors[0].movement,4);assert.equal(battle.actors[0].movementLeft,2);assert.equal(battle.actors[0].defense,6);assert.equal(battle.meta.statsSnapshotVersion,'16.78.110');
  for(let i=0;i<200;i++)P.decorateBattle(rt,battle);
  assert.equal(getValueCalls(),first,'repeated tactical renders/previews must reuse the in-memory snapshot');
  const beforeRows=getValueCalls();for(let i=0;i<200;i++)P.detailRows(battle.actors[0]);assert.equal(getValueCalls(),beforeRows,'detail rendering must be snapshot-only');
  P.markHeroDirty('hero');P.decorateBattle(rt,battle);assert.equal(getValueCalls(),first*2,'an explicit hero mutation must rebuild exactly one hero snapshot');
}
...
assert.equal((source.match(/api\.value\(/g)||[]).length,1,'canonical value() must only be called by the snapshot builder');
