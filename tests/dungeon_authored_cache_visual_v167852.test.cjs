...
function classList(){const s=new Set();return {add(v){s.add(v)},remove(...values){for(const v of values)s.delete(v)},contains(v){return s.has(v)}}}
...
const grid={dataset:{},querySelectorAll(q){if(q==='.dac167852Cache')return [];if(q==='.dav167870EnemyToken')return [];if(q===':scope > .dc047Cell')return cells;if(q==='.dc310Hero,.dc310Enemy')return [];return []}};
...
api.paint();
assert.equal(cells[1].classList.contains('dav167870WallCell'),true,'authored wall cell must receive current wall class');
