...
export function combatActorDisplayName(universe={},actor=null,id=null){
  ...
  if(actor.side==='heroes'){
    const hero=definitionById(universe.heroes,actor.metadata?.heroId||id);
    return hero?.name||actor.metadata?.heroId||String(id||'Héros');
  }
  const creature=definitionById(universe.bestiary,actor.metadata?.creatureId);
  return creature?.name||actor.metadata?.creatureId||String(id||'Ennemi');
}
...
export function renderCombatPresentation(universe={},combat=null,{journalLimit=8}={}){
  ...
  const cards=timeline.map(entry=>{
    ...
    return `<article class="combatant-card ${entry.active?'active':''} ${entry.ko?'ko':''}" data-combat-actor="${esc(entry.id)}"><div class="editor-card-head"><strong>${entry.side==='heroes'?'🛡️':'👹'} ${esc(entry.name)}</strong>${entry.ko?'<span class="status-pill">KO</span>':entry.active?'<span class="status-pill">Tour actif</span>':''}</div><div class="combat-chips">${resources.map(resource=>`<span class="combat-chip">${esc(resource.icon)} ${esc(resource.name)} <strong>${resource.current}${resource.max!=null?` / ${resource.max}`:''}</strong></span>`).join('')||'<span class="muted">Aucune ressource affichée</span>'}</div></article>`;
  }).join('');
  const journal=combatJournalEntries(universe,combat,{limit:journalLimit});
  return `<div class="combat-timeline" aria-label="Ordre des tours">${timeline.map(entry=>`<span class="combat-turn ${entry.active?'active':''} ${entry.ko?'ko':''}">${esc(entry.name)}${entry.ko?' · KO':''}</span>`).join('')}</div><div class="combat-grid">${cards}</div><div class="combat-journal"><h4>Journal moteur</h4>${journal.length?journal.slice().reverse().map(entry=>`<div class="combat-log-row"><span>${entry.seq!=null?`#${entry.seq}`:'•'}</span><strong>${esc(entry.text)}</strong></div>`).join(''):'<p class="muted">Aucune action résolue pour le moment.</p>'}</div>`;
}
