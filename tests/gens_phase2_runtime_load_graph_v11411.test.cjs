...
assert.equal(injected.length,21,'GitHub Pages must inject the current 21-module production list');
...
assert.equal(new Set(productionDirect).size,24,'production composition must expose 24 unique direct local JS entries');
...
assert.equal(allJs.length,83,'physical JS inventory must be Phase 2 baseline plus eight Phase 3 entries and three Phase 4 services');
assert.equal(phase2Js.length,72,'Phase 2 baseline JS inventory size drifted');
assert.equal(reachable.size,68,'production-reachable JS graph must currently contain 68 files');
