/* GenSrpG Dungeon — authored cache/runtime visual bridge 16.78.69.
   Built World Builder only: paints the actual authored room terrain in the live tactical board.
   It reads the original custom room cells/theme directly, so walls and floor themes are no longer
   inferred from the flattened compatibility map. No movement, combat, timeline, spawn or geometry logic changes. */
(function(){
"use strict";
const ROOT=typeof window!=="undefined"?window:globalThis;
const DOC=typeof document!=="undefined"?document:null;
const RT_KEY="gensrpg_dungeon_runtime_v2";
const VERSION="1.2.0",APP_VERSION="16.78.69";
...
function paintBoardVisuals(x,grid,cells){
  ...
  for(const cell of cells){cell.classList?.remove("dav167869WallCell");if(cell?.style)cell.style.removeProperty("color")}
  ...
  for(let i=0;i<cells.length;i++){
    ...
    if(wall){cells[i].classList?.add("dav167869WallCell");setBg(cells[i],WALL_ASSET);continue}
  ...
  if(grid.dataset){grid.dataset.dav167869Theme=theme;grid.dataset.dav167869Room=String(room?.id||x?.last?.customRoomId||"")}
  return true;
}
...
ROOT.DungeonAuthoredCacheVisual167852={VERSION,APP_VERSION,FLOOR_THEMES,WALL_ASSET,floorAsset,authoredRoom,cacheCells,enemyCellIndexes,cleanupLegacyChestScenes,paintBoardVisuals,paint,sync,install};
