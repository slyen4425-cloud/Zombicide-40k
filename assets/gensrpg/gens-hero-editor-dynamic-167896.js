/* GenSrpG V16.78.96 — canonical dynamic RPG stat block for Hero Editor. */
(function(){
"use strict";
const R=typeof window!=="undefined"?window:globalThis,D=typeof document!=="undefined"?document:null;
const VERSION="1.0.0",APP_VERSION="16.78.96";
...
let editorId=null,observer=null,scheduled=false;
...
function render(id=editorId){
 ...
 hideLegacy(host);
 let box=D.getElementById("hcCanonicalStatsFields167896");
 if(!box){box=D.createElement("div");box.id="hcCanonicalStatsFields167896";box.setAttribute("data-gens-canonical-hero-stats","1");box.style.display="contents";host.insertBefore(box,D.getElementById("hcRpgElementsWrap")||host.firstChild)}
 box.innerHTML="";
...
function read(){const out={};if(!D)return out;for(const input of D.querySelectorAll("#hcCanonicalStatsFields167896 [data-gens-canonical-hero-stat]")){const id=input.getAttribute("data-gens-canonical-hero-stat"),d=api()?.def?.(id);out[id]=num(input.value,d?.defaultValue||0)}return out}
...
function wrap(name,maker){const old=R[name];if(typeof old!=="function"||old.__heroDyn896)return false;const w=maker(old);w.__heroDyn896=true;w.__original=old;R[name]=w;return true}
function schedule(){if(scheduled)return;scheduled=true;setTimeout(()=>{scheduled=false;try{render(editorId)}catch(e){}},0)}
function observe(){if(!D||observer||typeof R.MutationObserver!=="function")return;observer=new R.MutationObserver(m=>{for(const x of m){const t=x.target;if(t?.id==="hcRpgStatsFields"||t?.closest?.("#hcRpgStatsFields")||[...x.addedNodes||[]].some(n=>n?.id==="hcRpgStatsFields"||n?.querySelector?.("#hcRpgStatsFields"))){schedule();break}}});observer.observe(D.documentElement,{childList:true,subtree:true})}
function install(){
 if(!api()){setTimeout(install,0);return false}
 wrap("openHeroCreator",old=>function(id){editorId=id??null;const out=old.apply(this,arguments);schedule();return out});
 wrap("hcRenderRpgStatsUsage",old=>function(){const out=old.apply(this,arguments);schedule();return out});
 ...
 observe();schedule();try{R.GENSRPG_VERSION=APP_VERSION}catch(e){}return true
}
R.GensHeroEditorDynamic167896={VERSION,APP_VERSION,LEGACY_INPUTS,activeDefs,render,read,syncLegacy,persist,install};
