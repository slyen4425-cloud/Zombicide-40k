...
const runtime={participants:['hero'],index:0,room:1,positions:{hero:0},last:{authoredRuntime167839:true,worldRuntime167823:true,worldDungeonId:'world-1',worldNodeId:'zone-1',map:{cells:['entry','floor','floor','floor','floor','floor','trap','floor','exit']},worldZoneContent167824:{mode:'fixed',traps:[trap]}}};
const store=storage({[RT]:JSON.stringify(runtime)});
let scene=[{id:'manual',kind:'trap',room:1,cellIndex:2,trapId:'dart',detected:false}],resolved=[];
...
let x=JSON.parse(store.getItem(RT));assert.equal(x.last.map.cells[6],'floor','le marqueur trap brut doit être neutralisé pour bloquer le tirage aléatoire');
let exact=scene.find(e=>e.exactTrap167845);assert.ok(exact,'un piège exact de scène doit être créé');assert.equal(exact.trapId,'rune');assert.equal(exact.detected,false,'le piège exact doit rester caché par défaut');assert.ok(scene.some(e=>e.id==='manual'),'un piège manuel étranger doit être conservé');
