...
function pixels(buffer){const png=PNG.sync.read(buffer);let n=0,white=0,sum=0;for(let i=0;i<png.data.length;i+=4){if(png.data[i+3]<16)continue;const r=png.data[i],g=png.data[i+1],b=png.data[i+2];n++;if(r>235&&g>235&&b>235)white++;sum+=(r+g+b)/3}return {whiteRatio:n?white/n:1,mean:n?sum/n:255,width:png.width,height:png.height}}
(async()=>{
  await new Promise(r=>server.listen(0,'127.0.0.1',r));
  const port=server.address().port,browser=await firefox.launch({headless:true});
  const context=await browser.newContext({viewport:{width:412,height:915},deviceScaleFactor:2.625,hasTouch:true,locale:'fr-FR'}),page=await context.newPage();page.setDefaultTimeout(15000);
  try{
    ...
    console.log(JSON.stringify({scenario:'Firefox wall representation matrix',asset:await page.evaluate(()=>window.GensRpgTacticalCombatV2Ui.WALL_ASSET),healthy,score,rows}));
