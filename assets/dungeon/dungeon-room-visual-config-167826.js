...
const VERSION="1.2.0",APP_VERSION="16.78.44",TEMPLATE_DUNGEON_ID="__room_template__";
...
function configureElement(dungeonId,nodeId,roomId,cell,obj,data){
  ...
  const content=clone(getContent(dungeonId,nodeId));if(content.mode==="inherit")content.mode="fixed";if(String(dungeonId)!==TEMPLATE_DUNGEON_ID){content.templateRoomId=String(roomId||"");content.templateLinked=false}
  ...
  if(obj==="enemy"||obj==="boss")spec={id:String(old?.id||uid("enemy")),enemyId:String(data?.enemyId||"dng_skeleton"),qty:Math.max(1,Math.min(50,Math.trunc(Number(data?.qty)||1))),cell:idx,role:obj==="boss"?"boss":"enemy",hasKey:!!data?.hasKey};
  ...
  else if(obj==="puzzle")spec={id:String(old?.id||uid("puzzle")),cell:idx,refId:String(data?.refId||""),targetType:String(data?.targetType||"cell"),targetIndex:Number.isFinite(Number(data?.targetIndex))?Math.max(0,Math.trunc(Number(data.targetIndex))):idx,label:String(data?.label||"")};
  list.push(spec);setList(content,obj,list);saveContent(dungeonId,nodeId,content);return clone(spec);
}
function removeElement(dungeonId,nodeId,cell,obj){const content=clone(getContent(dungeonId,nodeId));if(String(dungeonId)!==TEMPLATE_DUNGEON_ID){content.templateLinked=false}const idx=Number(cell),next=listForObject(content,obj).filter(x=>!(Number(x.cell)===idx&&((obj==="boss")?String(x.role||"enemy")==="boss":obj==="enemy"?String(x.role||"enemy")!=="boss":true)));setList(content,obj,next);saveContent(dungeonId,nodeId,content);return content}
function setMode(mode){const c=currentContext();if(!c)return false;const content=clone(getContent(c.dungeonId,c.nodeId));content.templateLinked=false;content.mode=["inherit","fixed","mixed"].includes(String(mode))?String(mode):"fixed";saveContent(c.dungeonId,c.nodeId,content);renderPanel();decorateGrid();return true}
...
function openEditor(cell,obj){
  ...
  if(obj==="enemy"||obj==="boss")body='<label>Monstre<select id="drv167826Enemy">'+enemyOptions(String(old?.enemyId||"dng_skeleton"))+'</select></label><label>Quantité<input id="drv167826Qty" type="number" min="1" max="50" value="'+Number(old?.qty||1)+'"></label><label style="align-content:end"><span><input id="drv167826Key" type="checkbox" '+(old?.hasKey?'checked':'')+'> Porte la clé</span></label>';
...
function saveActive(){
  ...
  if(activeObject==="enemy"||activeObject==="boss")data={enemyId:DOC.getElementById("drv167826Enemy")?.value,qty:DOC.getElementById("drv167826Qty")?.value,hasKey:!!DOC.getElementById("drv167826Key")?.checked};
  ...
  else if(activeObject==="puzzle")data={refId:DOC.getElementById("drv167826PuzzleRef")?.value,targetType:DOC.getElementById("drv167826Target")?.value,targetIndex:DOC.getElementById("drv167826TargetIndex")?.value,label:DOC.getElementById("drv167826PuzzleLabel")?.value};
