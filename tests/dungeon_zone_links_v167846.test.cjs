...
const graph={id:'world-1',nodes:[{id:'A',roomId:'ra',label:'Salle A'},{id:'B',roomId:'rb',label:'Salle B'}],edges:[{id:'e1',fromNodeId:'A',fromExitIndex:4,toNodeId:'B',toEntryIndex:0}],cacheBindings:[{id:'cb1',sourceNodeId:'A',sourceIndex:2,targetNodeId:'B',targetRoomId:'rb'}]};
...
function makeRuntime(node='B',pos=0){return {participants:['hero'],index:0,room:node==='A'?1:2,positions:{hero:pos},remaining:{hero:3},last:{worldRuntime167823:true,worldDungeonId:'world-1',worldNodeId:node,map:{cells:node==='A'?['entry','floor','cache','floor','exit']:['entry','floor','floor','floor','exit']}},world167823:{dungeonId:'world-1',heroNodes:{hero:node},nodeRooms:{A:1,B:2},roomNodes:{1:'A',2:'B'},history:{hero:['A','B']}}};}
...
const world={getConfig(){return {enabled:true,dungeonId:'world-1'}},ensureWorldState(x){return x.world167823},directFixedNode(x,g,plan,hero,w){const target=plan.targetNodeId,roomNo=w.nodeRooms[target];x.room=roomNo;w.heroNodes[hero]=target;x.last={worldRuntime167823:true,worldDungeonId:g.id,worldNodeId:target,map:{cells:target==='A'?['entry','floor','cache','floor','exit']:['entry','floor','floor','floor','exit']}};x.positions[hero]=plan.edge.toEntryIndex;store.setItem(RT,JSON.stringify(x));return true}};
...
assert.equal(api.reverseEdge(graph,'B',0).id,'e1','B entry must expose reverse traversal to A');
...
rt=JSON.parse(store.getItem(RT));assert.equal(rt.last.worldNodeId,'B');assert.equal(rt.positions.hero,0);assert.equal(rt.world167846ReturnStacks.hero.length,1,'cache travel must push a return frame');
