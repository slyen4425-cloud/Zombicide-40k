/* GenSrpG V16.78.22 — custom dungeon runtime bridge.
...
function clearBranch(x,id){try{return spatial()?.clearBranch?.(x,id)||x}catch(e){return x}}
...
function activeMeta(x=readRt()){return x?.dc322&&x.dc322.graphId?x.dc322:null}
...
function showToast(text){try{return ROOT.showToast?.(text)}catch(e){}}
...
function isResolved(x,key){return !!x?.dc322?.resolved?.[key]}
function markResolved(x,key){x.dc322.resolved=x.dc322.resolved||{};x.dc322.resolved[key]=Date.now();writeRt(x)}
...
function openPuzzle(x,nodeId,att,mode,sceneId){
  ...
  x.dc322.puzzleAttempts=x.dc322.puzzleAttempts||{};
  if(mode==="chest"){
    const list=arr(ROOT.loadDungeonSceneElements?.()),el=list.find(e=>String(e.id)===String(sceneId));
    if(!el)return false;
    el.challenge=clone(ch);el.challengeDone=false;el.challengeAttempts=Number(x.dc322.puzzleAttempts[key])||0;el.dc322PuzzleKey=key;el.dc322NodeId=nodeId;
  ...
  x.last.map.objective={type:"challenge",status:"locked",title:"Défi personnalisé",challenge:clone(ch),challengeAttempts:Number(x.dc322.puzzleAttempts[key])||0,dc322PuzzleKey:key,dc322NodeId:nodeId,dc322Cell:mode==="cell"};
...
function syncPuzzleCompletion(x){
  if(!x?.dc322)return false;let changed=false;
  const obj=x?.last?.map?.objective;
  if(obj?.dc322PuzzleKey){
    x.dc322.puzzleAttempts=x.dc322.puzzleAttempts||{};x.dc322.puzzleAttempts[obj.dc322PuzzleKey]=Number(obj.challengeAttempts)||0;
    if(obj.status==="done"){
      x.dc322.resolved=x.dc322.resolved||{};x.dc322.resolved[obj.dc322PuzzleKey]=Date.now();
      delete x.last.map.objective;delete x.last.objective;changed=true;
    }
  }
  const scenes=arr(ROOT.loadDungeonSceneElements?.());
  scenes.forEach(el=>{
    if(el?.dc322PuzzleKey){
      x.dc322.puzzleAttempts=x.dc322.puzzleAttempts||{};x.dc322.puzzleAttempts[el.dc322PuzzleKey]=Number(el.challengeAttempts)||0;
      if(el.challengeDone&&!x.dc322.resolved?.[el.dc322PuzzleKey]){x.dc322.resolved=x.dc322.resolved||{};x.dc322.resolved[el.dc322PuzzleKey]=Date.now();changed=true}
    }
  });
...
function mapForZone(z,nodeId){
  ...
  return {version:322,size,cells,entryIdx,exitIdx,dc322NodeId:nodeId,dc322Width:w,dc322Height:h,dc322EnemyCount:enemyCount,dc322BossCount:bossCount};
}
...
function cleanupOldCustomArtifacts(){
  try{ROOT.saveDungeonSceneElements?.(arr(ROOT.loadDungeonSceneElements?.()).filter(e=>!e?.dc322Custom))}catch(e){}
  try{ROOT.saveActiveEnemies?.(arr(ROOT.loadActiveEnemies?.()).filter(e=>!e?.dc322Custom))}catch(e){}
}
function sceneByCustom(nodeId,originalIndex,kind){return arr(ROOT.loadDungeonSceneElements?.()).find(e=>e?.dc322Custom&&String(e.dc322NodeId)===String(nodeId)&&Number(e.dc322OriginalIndex)===Number(originalIndex)&&(!kind||e.kind===kind))||null}
function instantiateScenes(x,nodeId,z,isBranch){
  const graph=activeGraph(x),room=z.room,roomNo=nodeRoom(x.dc322,nodeId),atts=syntheticAttachments(z);
  arr(room.cells).forEach((c,i)=>{
    ...
    if(obj==="chest"&&!sceneByCustom(nodeId,i,"chest")){
      ...
      const data={kind:"chest",name:"Coffre personnalisé",rarity,trapped:false,room:roomNo,cellIndex:ri,fromMap:true,dc322Custom:true,dc322RunId:x.dc322.runId,dc322NodeId:nodeId,dc322OriginalIndex:i,dc322TrapKey:trap?attachmentKey(nodeId,trap):""};
      if(puzzle){const key=attachmentKey(nodeId,puzzle),ch=challengeFor(puzzle,key);if(ch){data.challenge=ch;data.challengeDone=isResolved(x,key);data.dc322PuzzleKey=key}}
      try{ROOT.addDungeonSceneElement?.(data)}catch(e){}
    }else if(obj==="merchant"&&!sceneByCustom(nodeId,i,"merchant")){
      try{ROOT.addDungeonSceneElement?.({kind:"merchant",name:"Marchand",room:roomNo,cellIndex:ri,dc322Custom:true,dc322RunId:x.dc322.runId,dc322NodeId:nodeId,dc322OriginalIndex:i})}catch(e){}
      ...
      try{ROOT.addDungeonSceneElement?.({kind:"trapdoor",name:"Cache / passage secret",room:roomNo,cellIndex:ri,environment:"dungeon",branchType:"secret",dc322Custom:true,dc322Cache:true,dc322RunId:x.dc322.runId,dc322NodeId:nodeId,dc322OriginalIndex:i,dc322TargetNodeId:String(bind?.targetNodeId||"")})}catch(e){}
    }
  });
}
function spawnZoneEnemies(x,nodeId,z,isBranch){
  const roomNo=nodeRoom(x.dc322,nodeId),map=x.last?.map,depth=graphDepth(activeGraph(x),nodeId),normal=Math.max(0,Number(map?.dc322EnemyCount)||0),boss=Math.max(0,Number(map?.dc322BossCount)||0);
  const before=new Set(arr(ROOT.loadActiveEnemies?.()).map(e=>String(e.id)));
  try{if(normal>0)ROOT.dungeonEncounter?.(depth);if(boss>0)ROOT.dungeonBossRoom?.(depth)}catch(e){console.warn("DC322 spawn",e)}
  try{
    const all=arr(ROOT.loadActiveEnemies?.());
    all.forEach(e=>{if(!before.has(String(e.id))){e.dungeonRoom=roomNo;e.dc200Branch=!!isBranch;e.dc322Custom=true;e.dc322RunId=x.dc322.runId;e.dc322NodeId=nodeId}});
    ROOT.saveActiveEnemies?.(all);
  }catch(e){}
}
function retagZoneEnemies(x,nodeId,isBranch){
  const roomNo=nodeRoom(x.dc322,nodeId);try{const all=arr(ROOT.loadActiveEnemies?.());let ch=false;all.forEach(e=>{if(e?.dc322Custom&&e.dc322RunId===x.dc322.runId&&String(e.dc322NodeId)===String(nodeId)){if(e.dungeonRoom!==roomNo||!!e.dc200Branch!==!!isBranch){e.dungeonRoom=roomNo;e.dc200Branch=!!isBranch;ch=true}}});if(ch)ROOT.saveActiveEnemies?.(all)}catch(e){}
}
function applyRestIfNeeded(x,nodeId,z){
  if(x.dc322.restApplied?.[nodeId])return;const isRest=z?.room?.roomType==="rest"||arr(z?.room?.cells).some(c=>c?.object==="rest");if(!isRest)return;
  x.dc322.restApplied=x.dc322.restApplied||{};x.dc322.restApplied[nodeId]=true;
  try{
...
function captureZoneState(x=readRt()){
  if(!x?.dc322||!x.last)return x;const nodeId=currentNodeId(x);if(!nodeId)return x;
  x.dc322.zoneStates=x.dc322.zoneStates||{};
  x.dc322.zoneStates[nodeId]={last:clone(x.last),enemyCells:clone(x.enemyCells||{}),updatedAt:Date.now()};
  writeRt(x);return x;
}
function restoreZoneState(x,nodeId){const s=x?.dc322?.zoneStates?.[nodeId];if(!s)return false;x.last=clone(s.last);x.enemyCells=clone(s.enemyCells||{});return true}
function initZone(x,nodeId,isBranch){
  const z=zone(nodeId);if(!z?.room)throw new Error("Pièce du donjon introuvable : "+nodeId);
  x.dc322.initialized=x.dc322.initialized||{};x.dc322.zoneStates=x.dc322.zoneStates||{};
  if(!restoreZoneState(x,nodeId)){
    const map=mapForZone(z,nodeId);x.last={kind:zoneKind(z),room:Number(x.room)||0,at:Date.now(),title:z.node.label||z.room.name||"Pièce personnalisée",map,exitLocked:map.dc322BossCount>0,keyEnemyId:""};x.enemyCells={};
  }
  instantiateScenes(x,nodeId,z,isBranch);
  if(!x.dc322.initialized[nodeId]){spawnZoneEnemies(x,nodeId,z,isBranch);x.dc322.initialized[nodeId]=Date.now();applyRestIfNeeded(x,nodeId,z)}else retagZoneEnemies(x,nodeId,isBranch);
...
function enterNodeViaEdge(edge){
  ...
  x.positions=x.positions||{};x.remaining=x.remaining||{};x.positions[hero]=roomRuntimeIndex(z.room,edge.toEntryIndex);x.remaining[hero]=movement;x.dc313LastTransition={heroId:hero,from:Number(edge.fromNodeId?nodeRoom(meta,edge.fromNodeId):0),to:targetNo,created:!existing,at:Date.now(),dc322:true};
...
function customExplore(){
  let x=readRt();const graph=activeGraph(x);if(!x?.dc322||!graph)return native?.explore?.();
  syncPuzzleCompletion(x);x=readRt();
  if(!canAct(x))return modal("⏳ Action refusée","Ce n'est pas le tour du héros contrôlé par ce téléphone.");
  if(x.branch?.active&&x.branch?.dc322Custom){
  ...
  const buttons=edgeLabels(graph,edges).map(v=>'<button type="button" class="dc01Btn primary" style="width:100%;margin-top:7px" onclick="dc322TraverseEdge(\''+esc(v.edge.id)+'\')">🚪 '+esc(v.label)+'</button>').join("");
  return modal("🚪 Choisir une sortie",buttons);
}
function customChest(id){
  let x=readRt();if(x?.dc322){syncPuzzleCompletion(x);x=readRt()}const scenes=arr(ROOT.loadDungeonSceneElements?.()),el=scenes.find(e=>String(e.id)===String(id));if(!x?.dc322||!el?.dc322Custom)return native?.searchChest?.(id);
  ...
  const nodeId=String(el.dc322NodeId||currentNodeId(x)),original=Number(el.dc322OriginalIndex);
  if(!processTarget(x,nodeId,"chest",original,"chest",id))return false;
  return native?.searchChest?.(id);
}
function enterCustomBranch(id){
  let x=readRt();const scene=arr(ROOT.loadDungeonSceneElements?.()).find(e=>String(e.id)===String(id));if(!x?.dc322||!scene?.dc322Custom||!scene?.dc322Cache)return native?.enterBranch?.(id);
  const targetId=String(scene.dc322TargetNodeId||"");if(!targetId)return modal("🕳️ Cache","Cette cache n'est pas encore affectée à une instance de sous-pièce dans le constructeur.");
  const sourceNodeId=String(scene.dc322NodeId||currentNodeId(x)),targetNo=nodeRoom(x.dc322,targetId),z=zone(targetId);if(!targetNo||!z)return modal("🕳️ Cache","La sous-pièce liée n'est plus disponible.");
  ...
  const hero=activeHero(x),movement=preserveMovement(x,hero);x.room=targetNo;x.heroRooms=x.heroRooms||{};x.heroRooms[hero]=targetNo;x.branch.dc322Custom=true;x.branch.dc322SourceNodeId=sourceNodeId;x.branch.dc322TargetNodeId=targetId;x.branch.parentRoom=nodeRoom(x.dc322,sourceNodeId);x.branch.sourceId=String(id);x.branch.sourceCell=Number(scene.cellIndex);x.last=null;x.enemyCells={};
...
function leaveCustomBranch(){
  let x=readRt();if(!x?.branch?.active||!x.branch.dc322Custom)return native?.leaveBranch?.();
  const targetId=String(x.branch.dc322TargetNodeId||currentNodeId(x)),sourceNodeId=String(x.branch.dc322SourceNodeId||""),sourceId=String(x.branch.sourceId||"");captureZoneState(x);
  ...
  if(x?.dc322){retagZoneEnemies(x,targetId,true);captureZoneState(x);writeRt(x)}native?.render?.();sync();modal("↩️ Retour",heroName(activeHero(x))+" revient dans <strong>"+esc(zone(sourceNodeId)?.node?.label||zone(sourceNodeId)?.room?.name||"la pièce précédente")+"</strong>.");return true;
}
function cellInteraction(){
  let x=readRt();if(!x?.dc322)return false;const nodeId=currentNodeId(x),z=zone(nodeId),hero=activeHero(x),original=runtimeOriginalIndex(z?.room,Number(x.positions?.[hero]));if(original<0)return false;
...
function patchCellCard(x){
  if(!DOC)return;const host=DOC.getElementById("dc200Scene")||DOC.getElementById("dc02Scene");if(!host)return;let card=DOC.getElementById("dc322CellInteraction");const nodeId=currentNodeId(x),z=zone(nodeId),hero=activeHero(x),original=runtimeOriginalIndex(z?.room,Number(x.positions?.[hero]));const pending=original>=0?pendingAttachments(x,nodeId,"cell",original):[];
  if(!pending.length){card?.remove?.();return}
  if(!card){card=DOC.createElement("div");card.id="dc322CellInteraction";card.className="dc200SceneCard";host.appendChild(card)}
  const trap=pending.find(a=>a.kind==="trap"),puzzle=pending.find(a=>a.kind==="puzzle");const html='<div>'+(trap?'🪤':'🧩')+'</div><div class="grow"><strong>'+(trap?'Piège sur cette case':'Énigme sur cette case')+'</strong><small>'+(trap?'Le piège se déclenche au contact.':'Résous l’énigme pour valider cette interaction.')+'</small></div><button type="button" onclick="dc322ResolveCellInteraction()">'+(trap?'⚠️ RÉSOUDRE':'🧩 RÉSOUDRE')+'</button>';if(card.innerHTML!==html)card.innerHTML=html;
  x.dc322.lastCell=x.dc322.lastCell||{};const visit=nodeId+":"+original;if(x.dc322.lastCell[hero]!==visit){x.dc322.lastCell[hero]=visit;writeRt(x);if(trap&&!DOC.querySelector?.("#dc200Modal.open,#dc01Modal.open"))setTimeout(()=>cellInteraction(),0)}
}
function patchMetaAndAction(x){
  if(!DOC||!x?.dc322)return;const graph=activeGraph(x),nodeId=currentNodeId(x),z=zone(nodeId),meta=DOC.getElementById("dc01RoomMeta");setText(meta,Number(x.room)===0?"Entrée · "+String(graph?.name||"Donjon personnalisé"):String(graph?.name||"Donjon personnalisé")+" · "+String(z?.node?.label||z?.room?.name||"Pièce")+" · positions individuelles");
  const b=DOC.getElementById("dc01Explore");if(!b)return;b.disabled=!canAct(x);b.onclick=()=>ROOT.dc322Explore();
  if(x.branch?.active&&x.branch.dc322Custom){const ex=arr(z?.room?.cells).findIndex(c=>c?.object==="exit"),at=ex>=0&&Number(x.positions?.[activeHero(x)])===roomRuntimeIndex(z.room,ex);setText(b,!tactical()||at?"↩️ RETOURNER À LA PIÈCE":"🕳️ REJOINDRE LA SORTIE DE LA SOUS-PIÈCE");return}
...
function sync(){
  if(syncBusy)return;syncBusy=true;try{let x=readRt();if(!x?.dc322){localStorage.removeItem(ACTIVE_KEY);return}syncPuzzleCompletion(x);x=readRt();captureZoneState(x);x=readRt();patchMetaAndAction(x);patchCellCard(x);injectBuilderPlay()}catch(e){console.warn("DC322 sync",e)}finally{syncBusy=false}
}
...
function injectBuilderPlay(){
  if(!DOC||!builder())return;const box=DOC.getElementById("drc300Validation");if(!box||DOC.getElementById("dc322PlayBuiltDungeon"))return;const btn=DOC.createElement("button");btn.id="dc322PlayBuiltDungeon";btn.type="button";btn.className="drc300Primary";btn.style.cssText="width:100%;margin-top:9px;padding:12px;font-size:15px";btn.textContent="▶️ TESTER / JOUER CE DONJON";btn.onclick=()=>{const id=selectedBuilderGraphId();if(id)launch(id);else if(typeof alert==="function")alert("Ouvre d’abord un donjon dans la bibliothèque.")};box.parentNode?.insertBefore(btn,box.nextSibling)
}
function wrapCore(){
  ...
  c.endTurn=function(){let x=readRt();if(x?.dc322)captureZoneState(x);const r=native.endTurn?.apply(this,arguments);x=readRt();if(x?.dc322&&x.branch?.active&&x.branch.dc322Custom){const nid=currentNodeId(x);restoreZoneState(x,nid);writeRt(x);native.render?.()}setTimeout(sync,0);return r};
...
function install(){
  ...
  ROOT.dc322Explore=customExplore;ROOT.dc322TraverseEdge=traverseEdge;ROOT.dc322ResolveCellInteraction=cellInteraction;ROOT.dc322Launch=launch;
...
ROOT.DungeonCustomRuntime167822={VERSION,RT_KEY,ACTIVE_KEY,launch,customExplore,traverseEdge,customChest,enterCustomBranch,leaveCustomBranch,cellInteraction,sync,mapForZone,pendingAttachments,attachmentKey,install};
