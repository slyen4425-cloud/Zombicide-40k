...
function control(text){return {textContent:text,value:'',dataset:{},style:style(),hidden:false,disabled:false,setAttribute(){},removeAttribute(){},click(){refreshClicks++},closest(sel){return sel.includes('button')?this:null}}}
...
const api=context.DungeonAuthoredActionFix167857;assert.ok(api);assert.equal(api.APP_VERSION,'16.78.59');assert.equal(api.builtSelected(),true);
...
for(const fn of listeners.click||[])fn({target:cell,preventDefault(){},stopImmediatePropagation(){}});assert.ok(timers.some(t=>t.ms===70),'movement must schedule the live Actualiser control');while(timers.length)timers.shift().fn();assert.equal(refreshClicks,1,'the real Actualiser control must be clicked once after movement');assert.ok(paints>1);
...
console.log('Dungeon authored action fix V16.78.59: OK');
