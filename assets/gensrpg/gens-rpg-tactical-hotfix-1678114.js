/* GenSrpG V16.78.114.1 — selective rollback after V114.
   Keeps only the live Dungeon enemy-vision/combat detection that proved useful.
   Wall rendering and dice/result presentation are handed back to the pre-V114 V112 layer.
   V113 room/sub-room combat scoping remains untouched. */
(function(root,factory){
  ...
  "use strict";
  const VERSION="1.1.0",APP_VERSION="16.78.114.1";
  const DEFAULT_ENEMY_VISION=3,MAX_VISION=12,HEARTBEAT_MS=450;
  const ROLLBACK_STYLE_ID="gensRpgTacticalHotfix16781141RollbackStyle";
  ...
  function scanDetection(rt=R,reason="runtime-v1141",force=false){
    ...
    try{if(typeof rt?.dc200StartCombat==="function")rt.dc200StartCombat(enemyIds,"auto-engage-v114")}catch(e){try{rt?.console?.warn?.("V16.78.114.1 auto engage",e)}catch(_){}}
    return enemyIds;
  }
  function scheduleDetection(rt=R,reason="runtime-v1141",force=false,delays=[0,80,180]){
  ...
  function bindBoard(rt=R){const D=doc(rt);if(!D?.addEventListener||boardBound)return !!D;const hit=ev=>{if(!ev?.target?.closest?.("#dc047RoomBoard .dc047Cell"))return;scheduleDetection(rt,"board-input-v1141",true)};D.addEventListener("pointerup",hit,false);D.addEventListener("click",hit,false);boardBound=true;return true}
  ...
  function ensureRollbackStyle(rt=R){
    const D=doc(rt);if(!D||D.getElementById?.(ROLLBACK_STYLE_ID))return !!D;
    const s=D.createElement("style");s.id=ROLLBACK_STYLE_ID;
    s.textContent='.gtv2113DiceRow,.gtv2114RollStage{display:none!important}';
    (D.head||D.documentElement||D.body)?.appendChild(s);return true;
  }
  function restorePriorVisuals(rt=R){
    const D=doc(rt);if(!D)return false;ensureRollbackStyle(rt);
    for(const stage of D.querySelectorAll?.(".gtv2114RollStage")||[])stage.remove?.();
    for(const card of D.querySelectorAll?.("[data-v114-roll]")||[])card.removeAttribute?.("data-v114-roll");
    for(const wall of D.querySelectorAll?.(".gtv2114Wall")||[]){
      wall.classList?.remove?.("gtv2114Wall");
      for(const p of ["background-color","font-size","color","text-shadow"])wall.style?.removeProperty?.(p);
    }
    try{coherence(rt)?.markWallCells?.(rt)}catch(e){}
    try{coherence(rt)?.patchDice?.(rt)}catch(e){}
    return true;
  }
  function maintain(rt=R){queued=false;ensureDetectionHooks(rt);restorePriorVisuals(rt);return true}
  ...
  function observe(rt=R){
    ...
    observer=new rt.MutationObserver(muts=>{let board=false,dice=false;for(const m of muts){for(const n of m.addedNodes||[]){if(boardTouched(n))board=true;if(n?.nodeType===1&&(n.matches?.(".gtv2DiceBackdrop,.gtv2DiceCard")||n.querySelector?.(".gtv2DiceBackdrop,.gtv2DiceCard")))dice=true}if(board&&dice)break}if(board||dice)queueMaintain(rt);if(board)scheduleDetection(rt,"board-mutation-v1141",false,[0,100])});
    observer.observe(D.body,{childList:true,subtree:true});return true;
  }
  function startHeartbeat(rt=R){
    if(heartbeat!=null||typeof setInterval!=="function")return !!heartbeat;
    heartbeat=setInterval(()=>{if(currentBattle(rt))return;const D=doc(rt);if(!D?.querySelector?.("#dc047RoomBoard"))return;ensureDetectionHooks(rt);scanDetection(rt,"heartbeat-v1141",false)},HEARTBEAT_MS);return true;
  }
  function install(rt=R){ensureDetectionHooks(rt);bindBoard(rt);observe(rt);restorePriorVisuals(rt);scheduleDetection(rt,"install-v1141",false,[0,120]);startHeartbeat(rt);try{rt.GENS_RPG_TACTICAL_HOTFIX_VERSION=APP_VERSION}catch(e){}installed=true;return true}
  ...
  const api={VERSION,APP_VERSION,DEFAULT_ENEMY_VISION,MAX_VISION,HEARTBEAT_MS,runtimeState,heroScope,enemyScope,enemyVision,lineOfSight,detectionPairs,detectionSignature,scanDetection,scheduleDetection,ensureDetectionHooks,restorePriorVisuals,install,installWithRetries,status:()=>({installed,observer:!!observer,heartbeat:heartbeat!=null,wrappedGlobals:[...wrappedGlobals.keys()],wrappedCore:[...wrappedCore.keys()]})};
