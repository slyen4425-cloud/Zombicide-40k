...
(async()=>{
  ...
  try{
    ...
    for(let round=0;round<3;round++)for(const scale of scales){
      ...
      const box=await wall.boundingBox();assert.ok(box,`round ${round} scale ${scale}: wall bounding box missing`);const shot=await page.screenshot({animations:'disabled'}),p=regionPixels(shot,box);rows.push({round,scale,...p});
    }
    for(const row of rows){assert.ok(row.width>2&&row.height>2,`round ${row.round} scale ${row.scale}: empty wall region`);assert.ok(row.whiteRatio<0.55,`round ${row.round} scale ${row.scale}: wall became predominantly white (${row.whiteRatio.toFixed(3)})`);assert.ok(row.mean<225,`round ${row.round} scale ${row.scale}: wall became visually washed out (${row.mean.toFixed(1)})`)}
