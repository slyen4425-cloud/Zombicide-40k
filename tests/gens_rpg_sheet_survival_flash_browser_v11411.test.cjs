...
(async()=>{
  ...
  try{
    ...
    const trace=await page.evaluate(()=>{window.__rpgSheetSnap('final');window.__rpgSheetObserver?.disconnect?.();return {clicked,trace:window.__rpgSheetTrace||[],calls:window.__rpgSheetCalls||[],family:window.GensSurvivalModeIsolation1678104?.storedFamily?.()||'',active:window.activeGameProfileId?.()||'',dungeonMode:!!window.isDungeonMode?.()}}).catch(()=>null);
