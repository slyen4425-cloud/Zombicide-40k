/* GenSrpG Dungeon — authored branch navigation cleanup 16.78.63.
   While a hero is inside a secondary cache branch, keep only the dedicated return button.
   Hides stale legacy room/exit navigation controls that can route to an authored room exit.
   Restores them automatically outside the branch. No movement, combat, content or grid changes. */
(function(){
"use strict";
const ROOT=typeof window!=="undefined"?window:globalThis;
const DOC=typeof document!=="undefined"?document:null;
const RT_KEY="gensrpg_dungeon_runtime_v2";
const VERSION="1.0.0",APP_VERSION="16.78.63";
let installed=false;
...
function isLegacyBranchNav(el){if(!el||el.id==="dwr167846Return"||el.id==="dwr167846Cache")return false;const t=norm(el.textContent||el.value);if(t.includes("SORTIE NON RELIÉE"))return true;if(/^↩?\s*(RETOUR\s+)?SALLE\s*\d+\b/.test(t))return true;return false}
...
function sync(){const active=branchReturnActive();for(const el of candidates()){if(isLegacyBranchNav(el)){if(active)hide(el);else restore(el)}}const explore=DOC?.getElementById?.("dc01Explore");if(explore){if(active)hide(explore);else restore(explore)}return active}
function block(e){if(!branchReturnActive())return;const el=e?.target?.closest?.("button,[role='button'],input[type='button'],input[type='submit'],.btn,.dc01Btn,.dc047Btn");if(!el||el.id==="dwr167846Return")return;if(el.id==="dc01Explore"||isLegacyBranchNav(el)){e.preventDefault?.();e.stopImmediatePropagation?.();sync()}}
...
ROOT.DungeonAuthoredBranchNavCleanup167863={VERSION,APP_VERSION,readRt,activeHero,currentNode,branchReturnActive,isLegacyBranchNav,sync,install};
