...
const statSrc=fs.readFileSync(path.join(root,'assets','gensrpg','gens-custom-stat-runtime-profile-167889.js'),'utf8');
const equipmentSrc=fs.readFileSync(path.join(root,'assets','dungeon','dungeon-equipment-ui.js'),'utf8');
const setSrc=fs.readFileSync(path.join(root,'assets','dungeon','dungeon-set-editor-167818.js'),'utf8');
...
let itemOverrides={};
const items=[{id:'lucky_ring',name:'Anneau chanceux',type:'Équipement',rpgSlot:'ring',rpgBonuses:{chance:2},dungeonBuiltin:true}];
const store=new Map();
const localStorage={getItem:k=>store.get(k)||null,setItem:(k,v)=>{store.set(k,String(v));if(String(k).startsWith('state_')){const id=String(k).replace('state_','');states[id]=JSON.parse(v)}},removeItem:k=>store.delete(k)};
const ctx={console,Math,Date,localStorage,current:'hero',CHARS:{hero:{dungeonStats:{initiative:7}}},DUNGEON_EQUIPMENT_SETS:{},DUNGEON_ITEM_DEFINITIONS_316:items,
 ...
 dungeonItems:()=>items.map(i=>({...i,...(itemOverrides[i.id]||{})})),itemById:id=>ctx.dungeonItems().find(i=>i.id===id)||null,dungeonEquippedItems:()=>ctx.dungeonItems(),
 loadDungeonItemOverrides:()=>JSON.parse(JSON.stringify(itemOverrides)),saveDungeonItemOverrides:o=>{itemOverrides=JSON.parse(JSON.stringify(o))},
...
// Production order: equipment and sets load before the stat service.
vm.runInContext(equipmentSrc,ctx,{filename:'dungeon-equipment-ui.js'});
vm.runInContext(setSrc,ctx,{filename:'dungeon-set-editor-167818.js'});
vm.runInContext(statSrc,ctx,{filename:'gens-custom-stat-runtime-profile-167889.js'});
...
// The same unknown stat must be editable through equipment and set UIs, even though they loaded first.
assert.ok(ctx.DungeonEquipmentUI.fields().some(f=>f.key==='chance'),'equipment editor must expose authored Chance');
assert.ok(ctx.DungeonSetEditor.fields().some(f=>f.key==='chance'),'set editor must expose authored Chance');
let equipResult=ctx.DungeonEquipmentUI.setEquipmentBonus('lucky_ring','chance',3);assert.equal(equipResult.ok,true);assert.equal(itemOverrides.lucky_ring.rpgBonuses.chance,3);
let setResult=ctx.DungeonSetEditor.saveSetDefinition({id:'set_lucky',name:'Set chanceux',pieceCount:2,thresholds:[{pieces:2,bonuses:{chance:4}}]});assert.equal(setResult.ok,true);assert.equal(ctx.DUNGEON_EQUIPMENT_SETS.set_lucky.thresholds[0].bonuses.chance,4,'set bonus must preserve unknown stat id');
assert.match(ctx.DungeonSetEditor.summaryBonuses({chance:4}),/Chance/);
...
console.log('GenSrpG RPG stat service V16.79.01: unknown stat works through runtime + equipment + sets');
