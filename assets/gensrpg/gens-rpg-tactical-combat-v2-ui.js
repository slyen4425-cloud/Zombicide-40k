...
(function(root,factory){
  ...
  "use strict";
  const VERSION="0.5.0",APP_VERSION="16.78.107";
  ...
  const key=(x,y)=>`${x},${y}`;
  const sleep=ms=>new Promise(resolve=>R.setTimeout(resolve,ms));
  ...
  async function showRoll(result){
    ...
    const layer=R.document.createElement("div");layer.className="gtv2DiceBackdrop";layer.innerHTML=`<div class="gtv2DiceCard"><div class="gtv2DiceTitle">${esc(actorLabel(attacker))} → ${esc(actorLabel(target))}</div><div class="gtv2Die rolling">D100</div><div class="gtv2DiceResult">Jet en cours…</div></div>`;rootEl.appendChild(layer);
    await sleep(380);
    const die=layer.querySelector(".gtv2Die"),res=layer.querySelector(".gtv2DiceResult");if(die){die.classList.remove("rolling");die.textContent=String(result.roll)}if(res){res.className=`gtv2DiceResult ${ok?"ok":"fail"}`;res.textContent=ok?`✅ Touché · ${result.roll} ≤ ${result.hitChance} · ${result.damage} dégât(s)`:`❌ Raté · ${result.roll} > ${result.hitChance}`}
    await sleep(420);layer.remove();
  }
  async function runAiUntilHero(){
    ...
    while(battle?.status==="active"&&current()?.side==="enemy"&&guard++<50){
      ...
      if(r?.attack?.ok)await showRoll(r.attack);else await sleep(120);
