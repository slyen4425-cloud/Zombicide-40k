...
(async()=>{
  ...
  assert.deepEqual(finalChain.open,[
    ...
    'equipment-hotfix:__equipmentHotfix167817',
    'native'
  ],'Hero Editor retry must preserve one open owner below Cleanup');
  ...
  assert.deepEqual(finalChain.save,[
    ...
    'equipment-hotfix:__equipmentHotfix167817',
    'native'
  ],'Hero Editor retry must preserve one save owner below Cleanup invalidation');
  ...
  assert.equal(effects.saveCustomEquipmentCalls,1,
    'one user save must trigger one Hero Editor Dynamic persistence write after chain guard');
  ...
  console.log(JSON.stringify({
    ...
    finalChain,
    effects,
    finding:'chain-aware guard keeps one Hero Editor wrapper layer after retry; one save produces one persistence write'
  },null,2));
