...
assert.match(runtimeBootstrap,/gens-survival-mode-isolation-1678104\.js/,'Phase 6 characterization must start while production still references the legacy Survival isolation guard');
const mime={
...
const server=http.createServer((req,res)=>{
  ...
  if(pathname==='/assets/gensrpg/gens-survival-mode-isolation-1678104.js'){
    res.writeHead(200,{'content-type':'text/javascript; charset=utf-8','cache-control':'no-store'});
    res.end('window.__phase6LegacySurvivalIsolationBlocked=true;');
    return;
  }
...
(async()=>{
  let stage='boot';
  const mark=name=>{stage=name;console.log('[phase6-survival-without-legacy-guard]',name)};
  const watchdog=setTimeout(()=>{console.error('[phase6-survival-without-legacy-guard] WATCHDOG stage='+stage);process.exit(1)},120000);
  ...
  await context.addInitScript(()=>{
    try{
      ...
      localStorage.setItem('gensrpg_dungeon_runtime_v2',JSON.stringify({participants:['dungeon_aldren'],room:4,phase6Stale:true}));
      localStorage.setItem('gensrpg_dungeon_state_v1',JSON.stringify({room:4,phase6Stale:true}));
    }catch(e){}
  ...
  try{
    ...
    assert.equal(shell.family,'survival','Shell must record the Survival family without the legacy isolation guard');
    ...
    try{
      ...
      console.error('[phase6-survival-without-legacy-guard] family-render-diagnostic',JSON.stringify({familyDiag,browserErrors}));
      throw error;
    }
    ...
    assert.equal(shell.family,'survival','profile selection must preserve the Shell Survival family without the legacy isolation guard');
    ...
    try{
      ...
      console.error('[phase6-survival-without-legacy-guard] launch-diagnostic',JSON.stringify({launchDiag,dialogs,browserErrors}));
      throw error;
    }
    ...
    const launched=await page.evaluate(()=>({
      ...
      legacyGuard:typeof window.GensSurvivalModeIsolation1678104,
      legacyBlocked:window.__phase6LegacySurvivalIsolationBlocked===true
    }));

    assert.equal(launched.legacyGuard,'undefined','legacy Survival/Dungeon isolation guard must remain absent for the whole flow');
    assert.equal(launched.legacyBlocked,true,'fixture must have neutralized the production legacy guard');
    assert.equal(launched.active,'1','real Survival start must activate the common session');
    assert.equal(launched.family,'survival','real Survival start must keep the Shell Survival family without the legacy isolation guard');
    ...
    console.log(JSON.stringify({
      scenario:'Phase 6 Survival launch without legacy isolation guard',
