...
function configureElement(dungeonId,nodeId,roomId,cell,obj,data){
  ...
  if(obj==="enemy"||obj==="boss")spec={id:String(old?.id||uid("enemy")),enemyId:String(data?.enemyId||"dng_skeleton"),qty:Math.max(1,Math.min(50,Math.trunc(Number(data?.qty)||1))),cell:idx,role:obj==="boss"?"boss":"enemy",hasKey:!!data?.hasKey,hp:Number(data?.hp)>0?Number(data.hp):null,configured:true};
  else if(obj==="chest")spec={id:String(old?.id||uid("chest")),cell:idx,rarity:String(data?.rarity||"common"),gold:Math.max(0,Math.trunc(Number(data?.gold)||0)),items:Array.isArray(data?.items)?clone(data.items):parseItems(data?.itemsText||""),label:String(data?.label||""),configured:true};
  else if(obj==="trap")spec={id:String(old?.id||uid("trap")),cell:idx,trapType:String(data?.trapType||"damage"),damage:Math.max(0,Math.trunc(Number(data?.damage)||0)),refId:String(data?.refId||""),label:String(data?.label||""),once:data?.once!==false,configured:true};
  else if(obj==="puzzle")spec={id:String(old?.id||uid("puzzle")),cell:idx,refId:String(data?.refId||""),targetType:String(data?.targetType||"cell"),label:String(data?.label||""),configured:true};
...
function configuredAt(ctx,obj,cell){if(!ctx)return false;return specAt(getContent(ctx.dungeonId,ctx.nodeId),obj,cell)?.configured===true}
