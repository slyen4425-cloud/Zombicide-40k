/* GenSrpG V16.78.129 — targeted Dungeon UI cleanup without background observer.
   Built-in party-card art is repaired only from explicit Dungeon/UI render hooks.
   Also bootstraps the bounded Dungeon combat performance cache. */
(function(){
"use strict";
const R=typeof window!=="undefined"?window:globalThis,D=typeof document!=="undefined"?document:null;
const VERSION="1.3.0",APP_VERSION="16.78.129";
...
let installed=false,scheduled=false,repairingParty=false,perfRequested=false;
...
function ensureCombatPerformance(){if(!D||R.GensCombatRuntimePerformance1678129||perfRequested)return Boolean(R.GensCombatRuntimePerformance1678129);perfRequested=true;const s=D.createElement("script");s.src="assets/gensrpg/gens-combat-runtime-performance-1678129.js?v=1678129";s.async=false;s.dataset.gensCombatPerformance="1678129";s.onload=()=>{try{R.GensCombatRuntimePerformance1678129?.install?.()}catch(e){}};s.onerror=()=>{perfRequested=false};(D.head||D.documentElement).appendChild(s);return true}
function run(){ensureCombatPerformance();repairPartyCards();hideLegacyHeroInputs();cleanRuleSummary();return true}
...
function install(){if(installed)return true;installed=true;ensureCombatPerformance();hookAll();schedule();let tries=0;const retry=()=>{hookAll();if(tries++<12)setTimeout(retry,120)};setTimeout(retry,60);return true}
R.GensDungeonUiCleanup1678100={VERSION,APP_VERSION,ART,LEGACY_IDS,ensureCombatPerformance,repairPartyCards,hideLegacyHeroInputs,cleanRuleSummary,run,install};
