...
(async()=>{
  let stage='boot';
  const mark=name=>{stage=name;console.log('[capture-provider]',name)};
  const watchdog=setTimeout(()=>{console.error('[capture-provider] WATCHDOG stage='+stage);process.exit(1)},120000);
  ...
  try{
    ...
    try{
      ...
      console.error('[capture-provider] launch-diagnostic',JSON.stringify({diag,dialogs,browserErrors}));
      throw error;
    }
