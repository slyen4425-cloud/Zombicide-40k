...
const context={console,document,setTimeout(){return 1},MutationObserver:function(){this.observe=()=>{}},DungeonRoomVisualConfig167826:{decorateGrid(){}}};context.window=context;context.globalThis=context;vm.createContext(context);vm.runInContext(src,context,{filename:'dungeon-room-visual-hotfix-167827.js'});
const api=context.DungeonRoomVisualHotfix167827;assert.ok(api);assert.equal(api.APP_VERSION,'16.78.27');assert.equal(api.autoSelectVisibleContext(),true);assert.equal(changed,1);assert.equal(api.autoSelectVisibleContext(),true);assert.equal(changed,1);api.repairButton();assert.equal(btn.disabled,false);api.retireLegacyUi();assert.equal(legacy.style.display,'none');assert.equal(randomPanel.style.display,'none');
assert.match(loader,/dungeon-room-visual-hotfix-167827\.js\?v=167828/,'V16.78.28 doit toujours charger le hotfix de contexte');assert.match(loader,/dungeon-room-visual-config-167826\.js\?v=167828/);assert.match(sw,/gensrpg-cache-16\.78\.28-template-content/);assert.match(sw,/dungeon-room-visual-config-167826\.js/);assert.match(sw,/dungeon-room-visual-hotfix-167827\.js/);
const htmlArg=process.argv[2];if(htmlArg){const site=path.dirname(path.resolve(htmlArg));assert.ok(fs.existsSync(path.join(site,'assets','dungeon','dungeon-room-visual-hotfix-167827.js')));const builtLoader=fs.readFileSync(path.join(site,'assets','dungeon','dungeon-room-creator-feedback-167821.js'),'utf8');assert.match(builtLoader,/dungeon-room-visual-hotfix-167827\.js\?v=167828/)}
