...
const rt={
  console,
  localStorage:{getItem:k=>k==='gensrpg_session_family_guard_v1'?'survival':null},
  getActiveGameProfile:()=>({id:'dungeon',gameStyle:'dungeon'}),
  isDungeonMode:()=>false,
  GensSurvivalModeIsolation1678104:{rememberFamily:v=>{remembered=v}},
...
assert.equal(B.status(rt).dungeon,true,'active Dungeon profile must win over stale Survival family state');
assert.equal(rt.GENS_TACTICAL_V2_ROUTER_VERSION,'16.78.105');
...
assert.equal(opened,3);assert.equal(legacyLaunch,0,'direct legacy launch must be intercepted');

r=rt.startCombat(['e1'],'cell');
assert.equal(r.ok,true);assert.equal(opened,4);assert.equal(legacyDirect,0,'direct startCombat binding must be intercepted');
...
// Survival context still delegates to native/legacy behavior.
rt.getActiveGameProfile=()=>({id:'survival',gameStyle:'survival'});
rt.localStorage.getItem=()=> 'survival';
...
console.log('V16.78.105 Tactical V2 real entry routing OK');
