...
let generatorCalls=0,renderCalls=0,legacyConfigEnabledWrites=0,zoneApply=0,finalExitCalls=0;
...
const api=context.DungeonAuthoredRuntime167839;assert.ok(api);assert.equal(api.APP_VERSION,'16.78.76');assert.equal(api.VERSION,'1.0.2');assert.equal(api.active(),true);
...
(async()=>{
 ...
 button.onclick();x=read();assert.equal(generatorCalls,0);assert.equal(x.room,2);assert.equal(x.last.customRoomId,'room-b');assert.equal(x.last.worldNodeId,'B');assert.equal(x.last.kind,'boss');assert.equal(x.last.map.width,4);assert.equal(x.last.map.height,2);assert.deepEqual(x.last.map.cells,['entry','wall','boss','floor','floor','wall','floor','exit']);assert.equal(x.remaining.hero,2);
 assert.equal(legacyConfigEnabledWrites,0);
 core.render();assert.match(button.textContent,/TERMINER LE DONJON/,'une zone terminale doit afficher la fin du donjon, jamais sortie non reliée');
 core.explore();assert.equal(finalExitCalls,1,'le runtime principal doit appeler la procédure de fin sur la dernière zone');assert.equal(generatorCalls,0);
 localStorage.setItem(PRIMARY,JSON.stringify({kind:'adventure',id:'adv'}));core.explore();assert.equal(generatorCalls,1);
 assert.match(src,/private legacy explore\(\) cannot leak through/);assert.match(src,/btn\.onclick=travel/);assert.match(src,/movementForEntry/);assert.match(src,/authoredMovementReady167840/);assert.match(src,/TERMINER LE DONJON/);assert.doesNotMatch(src,/SORTIE NON RELIÉE|Sortie non reliée/,'le runtime principal ne doit plus exposer le message de sortie non reliée pour une zone terminale');assert.doesNotMatch(src,/generateDungeonMap\?\.|ROOT\.generateDungeonMap|dungeonEncounter\?\.|dungeonBossRoom\?\./);
 ...
 console.log('Dedicated authored Dungeon movement + direct final exit V16.78.76: OK');
