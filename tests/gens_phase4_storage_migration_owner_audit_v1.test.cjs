...
assert.equal(blob,'37387cf0582adaccab0206a2b746784976304600','storage audit must target the exact IndexedDB-raccord runtime blob');
...
const anonymous=scripts
  .filter(x=>!x.id)
  .map(x=>({...x,storageAccesses:storageAccessCount(x.body)}))
  .filter(x=>x.storageAccesses>0);
...
for(const token of ['GenSrpG_Assets','idbasset:','openDb','put','get','loadAllCache','externalizeSync','hydrate','externalizeAwait','exportAll','importAll']){
  assert.ok(service.includes(token),'Core IndexedDB service missing '+token);
}
...
for(const token of ['GENSRPG_IDB_MIGRATION_FLAG','gensrpgMigrateLocalImagesToIndexedDb','gensrpgBootstrapIndexedStorage']){
  assert.match(assetLegacy.body,new RegExp('\\b'+token+'\\b'),token+' must remain in the legacy migration/bootstrap owner');
}
