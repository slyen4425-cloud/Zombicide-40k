...
function uid(prefix){return String(prefix||"tpl")+"_"+Date.now().toString(36)+"_"+Math.random().toString(36).slice(2,8)}
function contentSignature(content){const c=clone(content)||emptyContent();delete c.updatedAt;return JSON.stringify(c)}
function defaultSpec(obj,cell){const i=Number(cell);if(obj==="enemy"||obj==="boss")return {id:uid("enemy"),enemyId:"dng_skeleton",qty:1,cell:i,role:obj==="boss"?"boss":"enemy",hasKey:false,configured:false};if(obj==="chest")return {id:uid("chest"),cell:i,rarity:"common",gold:0,items:[],label:"",configured:false};if(obj==="trap")return {id:uid("trap"),cell:i,trapType:"damage",damage:0,refId:"",label:"",once:true,configured:false};if(obj==="puzzle")return {id:uid("puzzle"),cell:i,refId:"",targetType:"cell",label:"",configured:false};return null}
...
function decorateGrid(){
  ...
  grid.querySelectorAll("[data-drc-index]").forEach(el=>{const i=Number(el.dataset.drcIndex),obj=String(r.cells?.[i]?.object||"");if(CONFIGURABLE.has(obj)){el.classList.add("drt167828Configurable");if(specAt(content,obj,i)?.configured===true)el.classList.add("drt167828Configured")}const handler=ev=>{block(ev);if(CONFIGURABLE.has(obj))ROOT.DungeonRoomTemplateContent167828?.openEditor?.(i,obj);else try{ROOT.showToast?.("Touche un ennemi, boss, coffre, piège ou énigme déjà placé.")}catch(e){}};el.__drt167828Handler=handler;el.onpointerdown=handler;if(typeof ROOT.PointerEvent==="undefined")el.onclick=handler});
}
...
function saveActive(){
  const id=inferRoomId();if(!id||activeCell<0)return false;let data={};if(activeObject==="enemy"||activeObject==="boss")data={enemyId:DOC.getElementById("drtEnemy")?.value,qty:DOC.getElementById("drtQty")?.value,hp:DOC.getElementById("drtHp")?.value,hasKey:!!DOC.getElementById("drtKey")?.checked};else if(activeObject==="chest"){const modern=ROOT.DungeonRoomContentUI167831?.selectedItems?.("template");data={rarity:DOC.getElementById("drtRarity")?.value,gold:DOC.getElementById("drtGold")?.value,items:Array.isArray(modern)?modern:(visualApi()?.parseItems?.(DOC.getElementById("drtItems")?.value||"")||[])};}else if(activeObject==="trap")data={trapType:DOC.getElementById("drtTrapType")?.value,damage:DOC.getElementById("drtDamage")?.value,refId:DOC.getElementById("drtTrapRef")?.value,label:DOC.getElementById("drtTrapLabel")?.value,once:true};else if(activeObject==="puzzle")data={refId:DOC.getElementById("drtPuzzleRef")?.value,targetType:DOC.getElementById("drtTarget")?.value,label:DOC.getElementById("drtPuzzleLabel")?.value};configureTemplate(id,activeCell,activeObject,data);closeEditor();renderPanel();decorateGrid();try{ROOT.showToast?.("✓ Contenu du modèle enregistré") }catch(e){}return true;
}
