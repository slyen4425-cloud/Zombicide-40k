...
function newRuntime(){return {participants:['hero'],index:0,round:1,room:0,last:null,positions:{hero:-1},remaining:{hero:3},enemyCells:{},roomStates:{},heroRooms:{hero:0}}}
...
const context={console,Math,Date,document,localStorage,setTimeout(fn){fn();return 1},showToast(){},activeDungeonAdventureId(){return 'adv'},dc305PositionalGameplay(){return true},DungeonCore01:core,DungeonSpatial313:spatial,DungeonZoneContent167824:zoneApi,
...
const api=context.DungeonAuthoredRuntime167839;assert.ok(api);assert.equal(api.APP_VERSION,'16.78.39');assert.equal(api.VERSION,'1.0.0');assert.equal(api.active(),true);
...
(async()=>{
 ...
 assert.equal(generatorCalls,0,'entrer dans un monde construit ne doit jamais appeler le générateur');assert.equal(x.room,1);assert.equal(x.last.authoredRuntime167839,true);assert.equal(x.last.worldGeneratorBypassed167839,true);assert.equal(x.last.customRoomId,'room-a');assert.equal(x.last.worldNodeId,'A');
 assert.equal(x.last.map.width,3);assert.equal(x.last.map.height,2);assert.deepEqual(x.last.map.cells,['entry','wall','chest','floor','wall','exit'],'la géométrie exacte Room Creator doit être chargée telle quelle');assert.equal(x.positions.hero,0);assert.equal(x.remaining.hero,3);
 ...
 x.positions.hero=5;write(x);core.render();assert.match(button.textContent,/Vers Boss exact/);assert.equal(button.onclick,api.travel,'chaque render legacy doit être réparé immédiatement par le moteur dédié');
 button.onclick();x=read();assert.equal(generatorCalls,0);assert.equal(x.room,2);assert.equal(x.last.customRoomId,'room-b');assert.equal(x.last.worldNodeId,'B');assert.equal(x.last.kind,'boss');assert.equal(x.last.map.width,4);assert.equal(x.last.map.height,2);assert.deepEqual(x.last.map.cells,['entry','wall','boss','floor','floor','wall','floor','exit']);
