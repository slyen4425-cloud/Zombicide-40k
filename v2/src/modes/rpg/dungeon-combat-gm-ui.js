...
export function renderDungeonCombatGmControls({universe={},combat=null,isGameMasterDevice=false}={}){
  const policy=combatInteractionPolicy(universe);
  if(!isGameMasterDevice||!policy.manualCombatControls||!combat||combat.phase!=='turn'||combat.metadata?.kind!=='dungeon-room-combat') return '';
...
export function mountDungeonCombatGmControls(root,{universe={},isGameMasterDevice=false,getCombat=()=>null,onChange=null,onError=null}={}){
  ...
  const html=renderDungeonCombatGmControls({universe,combat,isGameMasterDevice});
