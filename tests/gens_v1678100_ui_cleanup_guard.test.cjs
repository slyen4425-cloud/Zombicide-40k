...
assert.doesNotThrow(()=>new Function(src),'V16.78.106 cleanup syntax');
...
assert.match(sw,/gensrpg-cache-16\.78\.106-party-art-runtime-fix/);
...
const partyImg={alt:'',hidden:false,style:{display:'',visibility:'',setProperty(k,v){this[k]=v}},getAttribute(k){return k==='src'?partySrc:null},setAttribute(k,v){if(k==='src')partySrc=v;this[k]=v},tagName:'IMG'};
...
const document={readyState:'loading',addEventListener(){},getElementById(id){if(id==='dc01Heroes')return partyHost;if(id==='dungeonCombatFormula')return summary;return inputs[id]||null},createElement(){return {style:{setProperty(){}},setAttribute(){}}}};
const ctx={console,document,setTimeout,clearTimeout,MutationObserver:class{observe(){} disconnect(){}}};ctx.window=ctx;ctx.globalThis=ctx;
...
console.log('V16.78.106 guard: live party Aldren art + no hero stat duplicate + cleaned stat summary OK');
