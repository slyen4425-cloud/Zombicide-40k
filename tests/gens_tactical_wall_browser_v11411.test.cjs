...
(async()=>{
  ...
  try{
    ...
    assert.equal(result.asset,'assets/dungeon/creatures/dungeon_wall.png');
    for(const row of result.rows){assert.equal(row.count,1,`${row.id}: canonical wall painter must keep exactly one direct wall tile`);assert.equal(row.owner,true,`${row.id}: canonical owner class missing`);assert.equal(row.src,'assets/dungeon/creatures/dungeon_wall.png',`${row.id}: wrong wall asset`);assert.equal(row.complete,true,`${row.id}: wall image did not finish loading`);assert.ok(row.naturalWidth>0,`${row.id}: wall asset failed to decode`);assert.match(row.background,/dungeon_wall\.png/,`${row.id}: canonical wall background missing`)}
    ...
    assert.equal(result.lateRender.src,'assets/dungeon/creatures/dungeon_wall.png','late Dungeon wall must use the canonical wall asset');
