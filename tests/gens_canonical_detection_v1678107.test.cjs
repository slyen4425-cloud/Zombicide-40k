...
assert.match(src,/EXPLICIT_KEY="perceptionExplicit"/);
...
const profile={id:'u1',name:'Test',rpgUniverse:{stats:{active:['force'],dynamicDefinitions:[]},exploration:{trapDetectionRadius:2,enemyDetectionRadius:3},movement:{combatParticipationRange:3}}};
...
let saved=false;
const spatial={heroesHere(){return ['leader','near','far']},active(){return 'leader'},participatingHeroes(x,leader,maxRange,tactical=true){if(!tactical)return this.heroesHere(x);return this.heroesHere(x).filter(id=>id===leader||Number(x.distances[id])<=Number(maxRange))}};
const context={console,setTimeout(fn){fn();return 1},clearTimeout(){},window:null,current:'leader',currentRpgProfile(){return profile},getActiveGameProfile(){return profile},loadGameProfiles(){return [profile]},saveGameProfiles(){saved=true},activeGameProfileId(){return 'u1'},getActiveGameProfileId(){return 'u1'},loadState(id){return states[id]||{rpgAttributes:{}}},CHARS:{leader:{dungeonStats:{}},near:{dungeonStats:{}},far:{dungeonStats:{}},legacy:{dungeonStats:{}}},findCustomHero(){return null},DungeonSpatial313:spatial,activeEnemyDefinition(){return {id:'wolf',rule:{perception:2,rpgStats:{perception:6}}}},dungeonEnemyRpgStats(){return {perception:6}},GensEnemyCanonicalStats1678105:{enabled(){return true}},GensCleanRpgStats167874:{value(id,key){return Number(states[id]?.rpgAttributes?.[key]??0)},def(id){return id==='perception'?{id,name:'Détection',defaultValue:2}:null}}};
...
assert.equal(api.enemyRange({rule:{rpgStats:{perception:0}}}),3,'legacy zero still migrates to former effective universe radius');
assert.equal(api.enemyRange({rule:{perceptionExplicit:true,rpgStats:{perception:0}}}),0,'explicit zero must now mean no detection');
const marked=api.markExplicit({rule:{rpgStats:{perception:0}}});
assert.equal(marked.rule.perceptionExplicit,true);assert.equal(marked.rule.perception,0);
...
console.log('V16.78.108 canonical Detection: explicit zero + legacy migration + per-ally combat participation OK');
