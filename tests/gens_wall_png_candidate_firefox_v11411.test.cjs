...
const server=http.createServer((req,res)=>{const pathname=decodeURIComponent(new URL(req.url,'http://127.0.0.1').pathname);if(pathname==='/wall-candidate-test.html'){res.writeHead(200,{'content-type':'text/html; charset=utf-8','cache-control':'no-store'});res.end('<!doctype html><html><body><div id="grid" style="width:240px;transform-origin:0 0"><div id="wall" style="width:120px;height:120px;overflow:hidden"><img id="img" src="/assets/dungeon/creatures/dungeon_wall.png" style="display:block;width:100%;height:100%;object-fit:cover"></div></div></body></html>');return}const file=path.resolve(root,'.'+pathname);if(!file.startsWith(root+path.sep)){res.writeHead(403);res.end('forbidden');return}fs.readFile(file,(err,data)=>{if(err){res.writeHead(404);res.end('not found');return}res.writeHead(200,{'content-type':mime[path.extname(file).toLowerCase()]||'application/octet-stream','cache-control':'no-store'});res.end(data)})});
(async()=>{
  ...
  try{
    await page.goto(`http://127.0.0.1:${port}/wall-candidate-test.html`,{waitUntil:'load'});
