...
export function advanceCaptureModeTime(state,{amount=1,tickReactionCooldowns=true,tickStatuses=true}={}){
  ...
  return {
    ...
    reactionTime:reaction.reactionTime,
    statusTick:{
      playerApplied:clone(statuses.applied?.player||[]),
      opponentApplied:clone(statuses.applied?.opponent||[]),
      playerExpired:clone(statuses.expired?.player||[]),
      opponentExpired:clone(statuses.expired?.opponent||[]),
    },
