...
(async()=>{
 ...
 assert.equal(contentStore['world-authored'].A.mode,'inherit','une zone liée doit rester en héritage dynamique au premier passage');assert.equal(contentStore['__room_template__']['room-a'].mode,'fixed','le runtime doit lire le modèle de pièce sans le recopier dans la zone');assert.ok(zoneApply>=1);
