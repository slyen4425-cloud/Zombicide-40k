...
|---|---|---|---|---|
| Accueil / modes | Survie, RPG, Monster Capture, VS/PVP | Modes isolés | En cours | Navigation propre, aucun état gameplay partagé |
...
| Inventaire / objets RPG | Armes, armures, consommables, munitions, sets | Moteur objets + inventaire + consommables combat | Base active | Stats/effets/charges/restrictions génériques ; audit munitions/sons restant |
| Sets RPG | Set cuir + Armure des Anciens + paliers | Moteur de sets intégré aux héros/compétences | Base active | `setId`, paliers, bonus actifs et exposition UI à maintenir |
| Marchands RPG | Stock, rupture, revente, persistance | Moteur marchand V2 | Base active | Stock configurable et persistant, achat/revente cohérents |
| Progression RPG | XP, niveaux, points, popup | Moteur progression V2 | Base active | Courbe configurable + compétences/formes ; UX level-up à poursuivre |
| Bestiaire RPG | Créatures Dungeon éditables | Moteur bestiaire V2 + spawn/runtime salle | Base active | Stats/ressources/attaques/loot configurables ; IA avancée à poursuivre si besoin |
| MJ / IA RPG | IA, MJ, mixte | Tours ennemis auto + MJ total + panneau manuel sécurisé par appareil | Base active | Rôles clairs, outils MJ invisibles joueur, même combatState |
| Monster Capture — isolation | Legacy mélangeait certaines données/profils avec d'autres familles | `modes/capture/` séparé | Architecture cible corrigée | Aucun état mutable, inventaire, profil, créature ou sauvegarde partagé avec RPG/Survie/VS |
| Monster Capture — données | Créatures, biomes, taux d'apparition, équipe active, réserve | Aucun contrat Capture V2 encore | Absent | Namespace et stockage propres au mode Capture |
...
| Monster Capture — capture | % par créature, balles/niveaux, bonus PV faibles | À reconstruire dans moteur Capture | Absent | Formule configurable, objets de capture, statut capturé, persistance |
| Monster Capture — combats | VS IA/VS joueurs, charges/coûts, futur tactique dynamique | Moteur Capture dédié à définir | Absent | Ne dépend pas du combat RPG mutable ; partage possible uniquement de services Core neutres |
| Monster Capture — évolution | Créatures avec/sans évolution | Contrat Capture dédié | Absent | Évolution créature indépendante des formes héros RPG |
| Monster Capture — bootstrap | Seed global historique `ensureBuiltinMonsterCapture162()` | Interdit au démarrage global | Architecture cible corrigée | Initialisation uniquement à l'ouverture/initialisation de Capture |
| Audio | Sons répartis dans plusieurs systèmes | Bibliothèque/runtime audio centralisés | En cours | Asset audio réutilisable via Core neutre ; données de binding propres à chaque mode |
| Sauvegarde | localStorage + couches historiques + backup/import | Core stockage local + routeur remote | En cours | Providers neutres partagés, namespaces et migrations propres à chaque mode |
...
| Multijoueur | Entrée héberger/rejoindre + système Supabase historique | Aucun module V2 | Absent | Host/join sans fusionner les stockages gameplay des modes |
...
| Import / export utilisateur | Sauvegardes/imports historiques | Pas encore de flux utilisateur final | Absent | Export/import versionnés et isolés par mode |
...
| VS / PVP | Réservé mais non construit | Espace isolé | Réservé | Aucun couplage mutable au RPG/Capture/Survie |

## Manques confirmés par l'audit legacy du 2026-09-12

1. **Monster Capture** n'a pas encore de moteur V2, mais son architecture cible est désormais fixée : **mode complètement séparé**, au même niveau que Survie / RPG / VS-PVP.
...
4. La séparation Capture ne doit pas être seulement visuelle : profils, inventaires, créatures, progression, sauvegardes et migrations doivent utiliser leurs propres namespaces et contrats afin qu'une donnée Capture ne puisse plus écraser une donnée RPG ou Survie.
5. **Multijoueur** n'a pas encore de remplacement V2 alors que l'ancienne app exposait héberger/rejoindre et s'appuyait sur un système Supabase existant.
6. **PWA/cache** n'existe pas encore dans `v2/` : pas de manifest V2, pas de service worker V2, donc pas encore de stratégie d'installation/offline/version de cache pour la reconstruction.
7. **Sauvegarde utilisateur complète** reste incomplète : le provider local/remote existe, mais les flux finaux export/import/migration/rollback ne sont pas encore consolidés.
8. **Assets et sons legacy** doivent être inventoriés avant migration, sans recopier les doublons ni les wrappers historiques.
9. **Tests legacy pertinents** doivent être convertis en régressions V2 au fur et à mesure, plutôt que garder les anciens correctifs comme dépendances runtime.
...
7. Aucun mode ne doit lire/écrire directement les données gameplay mutables d'un autre mode.
