...
const rooms=[
  {id:'room_a',name:'Entrée antique',width:15,height:15,cells:Array.from({length:225},(_,i)=>({terrain:'floor',object:i===0?'entry':i===224?'exit':null}))},
...
const graph={id:'world_1',name:'Monde test',startNodeId:'A',nodes:[{id:'A',roomId:'room_a',label:'Entrée'},{id:'B',roomId:'room_b',label:'Gardes'},{id:'C',roomId:'room_c',label:'Crypte'}],edges:[{id:'ac',fromNodeId:'A',fromExitIndex:224,toNodeId:'C',toEntryIndex:0},{id:'ca',fromNodeId:'C',fromExitIndex:99,toNodeId:'A',toEntryIndex:0}]};
...
const ctx={console,Math,Date,localStorage,setTimeout(fn){fn();return 1},clearTimeout(){},globalThis:null,window:null,DungeonCore01:core,DungeonSpatial313:spatial,
  ...
  activeDungeonAdventureId(){return 'adv_1'},loadActiveEnemies(){return JSON.parse(JSON.stringify(enemies))},saveActiveEnemies(next){enemies=JSON.parse(JSON.stringify(next))},showToast(){},modal(){return false},dungeonRoomExitLocked102(){return false}
};
ctx.globalThis=ctx;ctx.window=ctx;vm.createContext(ctx);vm.runInContext(src,ctx,{filename:'dungeon-world-runtime-167823.js'});
const api=ctx.DungeonWorldRuntime167823;assert.ok(api);assert.equal(api.APP_VERSION,'16.78.34');
api.saveConfig({enabled:true,dungeonId:'world_1'},'adv_1');
for(const node of ['A','B','C'])api.saveZoneContent('world_1',node,{mode:'fixed',enemies:node==='B'?[{enemyId:'dng_skeleton',cell:40}]:[],chests:[]});
assert.equal(core.explore(),true,'first fixed zone should load directly');
let x=read();assert.equal(legacyExploreCalls,0,'fixed built-world zone must not call legacy generator');assert.equal(x.room,1);assert.equal(x.last.worldNodeId,'A');assert.equal(x.last.worldAuthoritativeDirect167834,true);assert.equal(x.last.map.size,15);assert.equal(x.last.map.cells.length,225);assert.equal(x.positions.hero,0);assert.equal(enemies.length,0,'no legacy enemy may leak into fixed zone');
...
x.positions.hero=99;write(x);assert.equal(core.explore(),true,'revisit should restore canonical room without generation');x=read();assert.equal(x.room,1);assert.equal(x.last.worldNodeId,'A');assert.equal(legacyExploreCalls,0);assert.equal(x.last.map.cells.length,225);
x.positions.hero=1;write(x);assert.equal(core.explore(),false,'unconnected route is blocked');assert.equal(read().room,1);
api.saveConfig({enabled:false,dungeonId:'world_1'},'adv_1');const before=read().room;core.explore();assert.equal(legacyExploreCalls,1);assert.equal(read().room,before+1,'disabled mode falls back to legacy explore');
...
if(htmlArg){const html=fs.readFileSync(htmlArg,'utf8');const p=html.indexOf('assets/dungeon/dungeon-world-runtime-167823.js?v=167823');assert.ok(p>=0,'final HTML must load authoritative world runtime')}
