/* GenSrpG Tactical Combat V2 — polished tactile battlefield UI.
   V16.78.114.7: single-owner wall renderer and short event-driven D100 animation.
...
(function(root,factory){
  ...
  "use strict";
  const VERSION="0.7.0",APP_VERSION="16.78.114.7";
  ...
  function preloadWall(){
    const doc=D();if(!doc||doc.getElementById("gensTacticalWallPreload1147"))return false;
    const link=doc.createElement("link");link.id="gensTacticalWallPreload1147";link.rel="preload";link.as="image";link.href=WALL_ASSET;(doc.head||doc.documentElement)?.appendChild(link);return true;
  }
  ...
  async function showDiceResult(attacker,target,result,phase){
    ...
    if(!doc.getElementById("gtv21147DiceStyle")){const st=doc.createElement("style");st.id="gtv21147DiceStyle";st.textContent='.gtv21147DiceRow{display:flex;gap:7px;justify-content:center;flex-wrap:wrap;margin:9px 0}.gtv21147DiceRow .gtv2Die{width:62px;height:62px;font-size:26px;animation:gtv21147Dice .18s cubic-bezier(.2,.78,.24,1) both}.gtv2DiceCard.gtv21147Rolling .gtv2DiceResult,.gtv2DiceCard.gtv21147Rolling .gtv2DiceRule strong,.gtv2DiceCard.gtv21147Rolling .gtv2112Explain{visibility:hidden!important}@keyframes gtv21147Dice{0%{transform:translateY(4px) rotate(-16deg) scale(.92)}48%{transform:translateY(-4px) rotate(12deg) scale(1.06)}100%{transform:none}}@media(max-width:430px){.gtv21147DiceRow .gtv2Die{width:54px;height:54px;font-size:23px}}@media(prefers-reduced-motion:reduce){.gtv21147DiceRow .gtv2Die{animation-duration:.01s}}';(doc.head||doc.documentElement||doc.body)?.appendChild(st)}
    ...
    await Promise.all(faces.map(face=>new Promise(resolve=>{let done=false;const finish=()=>{if(done)return;done=true;resolve()};face.addEventListener("animationend",finish,{once:true});face.addEventListener("animationcancel",finish,{once:true})})));
    faces.forEach((face,i)=>face.textContent=String(rolls[i]));card?.classList?.remove?.("gtv21147Rolling");if(resultEl)resultEl.style.visibility="visible";if(ruleStrong)ruleStrong.style.visibility="visible";if(button)button.disabled=false;
  ...
  function nearestOpponent(actor){return battle.actors.filter(a=>a.alive&&a.side!==actor.side).sort((a,b)=>E().distance(actor,a)-E().distance(b)||a.hp-b.hp)[0]||null}
  ...
  async function runAiUntilHero(){busy=true;let guard=0;while(battle?.status==="active"&&current()?.side==="enemy"&&guard++<50){const ai=current();notice=`Tour de ${actorDisplayName(ai)}.`;render();let target=nearestOpponent(ai),choice=target?bestAttack(ai,target):null;if(!choice&&target){const cell=bestApproach(ai,target);if(cell)E().moveActor(battle,ai.id,cell);target=nearestOpponent(ai);choice=target?bestAttack(ai,target):null}if(choice&&target&&ai.actionsLeft>0){const result=resolveVisibleAttack(ai,target,choice.a.id);await showDiceResult(ai,target,result,"Tour ennemi");render()}if(battle.status==="active")E().endTurn(battle);notice="";render()}busy=false;notice="";render()}
  async function doAttack(id){const cur=current(),target=targetActor();if(!cur||cur.side!=="hero")return;selectedAttack=id;const p=target?E().attackPreview(battle,cur.id,target.id,id):null;if(!target){notice="Choisis d’abord une cible.";render();return}if(!p?.ok){notice=previewReason(p);render();return}busy=true;notice="";const result=resolveVisibleAttack(cur,target,id);await showDiceResult(cur,target,result,"Tour du héros");render();if(battle.status==="active"&&cur.actionsLeft<=0)E().endTurn(battle);render();if(battle.status==="active"&&current()?.side==="enemy")await runAiUntilHero();else{busy=false;render()}}
  ...
  function open(input,opts={}){const doc=D();if(!doc)throw new Error("Tactical UI requires a document");if(!E())throw new Error("Tactical engine unavailable");preloadWall();ensureStyle();hookEditor();patchDungeonMapHtml();options=opts||{};battle=input?.actors?E().createBattle(input):A()?.createBattle?.(R,input||{});if(!battle)throw new Error("Cannot create tactical battle");battle.config=battle.config||{};battle.config.rollHighToHit=getRollHighToHit();rootEl=doc.createElement("div");rootEl.className="gtv2Overlay";rootEl.setAttribute("data-gens-tactical-v2","1");rootEl.addEventListener("click",onClick);doc.body.appendChild(rootEl);render();if(current()?.side==="enemy")void runAiUntilHero();return battle}
  function openCurrentEncounter(opts={}){return open(opts,opts)}
  function getBattle(){return battle}

  function patchDungeonMapHtml(){if(mapPatched)return true;const old=R.dungeonMapHtml;if(typeof old!=="function")return false;if(old.__gtv271WallPatch){mapPatched=true;return true}const w=function(map){const html=old.apply(this,arguments),doc=D();if(!doc||typeof html!=="string"||!html)return html;try{const tpl=doc.createElement("template");tpl.innerHTML=html;const cells=[...tpl.content.querySelectorAll(".dc047Grid > .dc047Cell")],kinds=arr(map?.cells);for(let i=0;i<cells.length;i++)if(String(kinds[i]||"").toLowerCase()==="wall"){cells[i].classList.add("dav167870WallCell");cells[i].style.setProperty("background",`#171512 url("${WALL_ASSET}") center/cover no-repeat`)}return tpl.innerHTML}catch(e){return html}};w.__gtv271WallPatch=true;w.__original=old;R.dungeonMapHtml=w;mapPatched=true;return true}
  function paintLiveWalls(){const doc=D();if(!doc)return 0;let count=0;const map=runtimeState()?.last?.map,kinds=arr(map?.cells),cells=[...(doc.querySelectorAll?.("#dc047RoomBoard .dc047Grid > .dc047Cell")||[])];for(let i=0;i<cells.length;i++)if(String(kinds[i]||"").toLowerCase()==="wall"&&!cells[i].classList?.contains?.("dav167870WallCell")){cells[i].classList?.add?.("dav167870WallCell");count++}return count}
  ...
  function installGlobalPolish(){preloadWall();ensureStyle();hookEditor();patchDungeonMapHtml();hookDungeonRender();paintLiveWalls();R.GENS_RPG_TACTICAL_UI_VERSION=APP_VERSION;return true}
