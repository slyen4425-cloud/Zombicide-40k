/* GenSrpG V16.78.78 — safe final exit for authored Dungeon worlds.
   A terminal authored zone ends only from its real EXIT cell. Other interactions (cache, chest,
   event, puzzle...) keep the normal explore flow. Closing the completion popup returns home. */
(function(){
"use strict";
const R=typeof window!=="undefined"?window:globalThis,D=typeof document!=="undefined"?document:null;
const RT_KEY="gensrpg_dungeon_runtime_v2";
const VERSION="1.1.0",APP_VERSION="16.78.78";
...
function finalState(){
  ...
  if(!a?.active?.()||!x||Number(x.room)<=0||x.branch?.active)return null;
  ...
  const hero=activeHero(x),map=x?.last?.map||{},cells=Array.isArray(map.cells)?map.cells:[];
  ...
  /* Une zone terminale sans vraie case EXIT n'est jamais terminable automatiquement. */
  const hasExit=exitIdx>=0&&String(cells[exitIdx]||"").toLowerCase()==="exit";
...
function scheduleHome(){
  /* Le modal Dungeon appelle render() après son callback de fermeture : le retour doit gagner APRES ce render. */
  for(const ms of [0,40,160])setTimeout(showHome,ms);
  return true;
}
function finish(){
...
function rememberNormalButton(btn){
  if(!btn||btn.dataset?.daf167878Final==="1")return;
  lastNormalButton={text:btn.textContent,disabled:!!btn.disabled,onclick:btn.onclick};
}
function restoreNormalButton(btn){
  if(!btn||btn.dataset?.daf167878Final!=="1")return false;
  if(lastNormalButton){btn.textContent=lastNormalButton.text;btn.disabled=lastNormalButton.disabled;btn.onclick=lastNormalButton.onclick}
  if(btn.dataset)delete btn.dataset.daf167878Final;
  return true;
}
function syncButton(){
  if(!D)return false;
  const btn=D.getElementById("dc01Explore");if(!btn)return false;
  const f=finalState();
  if(!f||!f.hasExit||!f.atExit){restoreNormalButton(btn);return false}
  rememberNormalButton(btn);
  if(btn.dataset)btn.dataset.daf167878Final="1";
  btn.disabled=false;
...
function wrapRender(name){const core=R.DungeonCore01,old=core?.[name];if(typeof old!=="function"||old.__daf167878)return false;const w=function(){const out=old.apply(this,arguments);try{syncButton()}catch(e){}return out};w.__daf167878=true;w.__original=old;core[name]=w;return true}
...
R.DungeonAuthoredFinalExit167875={VERSION,APP_VERSION,finalState,finish,syncButton,showHome,scheduleHome,install};
