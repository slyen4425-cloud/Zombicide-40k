...
assert.match(source,/APP_VERSION="16\.78\.102\.4"/);
...
assert.match(sw,/gensrpg-cache-16\.78\.102\.4-combat-detection-fastpath/);
...
let equipmentCalls=0,attributeCalls=0,canonicalCalls=0,enemyCalls=0,saves=0,statChanges=0,detectPaint=0,detectResolve=0,trapPaint=0;
const runtime={room:1,participants:["hero1"],positions:{hero1:4},enemies:[{id:"e1",hp:4,cell:5}]};
const sandbox={console,document,setTimeout:setTimeoutFake,clearTimeout:clearTimeoutFake,performance:{now:()=>clock},requestAnimationFrame:fn=>setTimeoutFake(fn,16),current:"hero1",state:{xp:20,rpgAttributes:{force:12}},navigator:{vibrate(){}},
...
assert.equal(attributeCalls,1,"HP/enemy persistence must not invalidate hero stat caches");
...
assert.equal(detectResolve,2,"detection must run again after the short duplicate window");
...
console.log("V16.78.102.4 combat/detection fast path OK",{d6DoneAt,d100DoneAt,equipmentCalls,attributeCalls,canonicalCalls,enemyCalls,detectPaint,detectResolve,metrics:sandbox.GensMobileCombatPerformance16781022.metrics()});
