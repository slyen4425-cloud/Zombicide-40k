...
export function interactionCheckOptions(checks=[],selected=null){
...
export function mountRoomEditor(host,universe={}){
  ...
  function render(){
    ...
    host.querySelectorAll('[data-interaction-id]').forEach(card=>{
      ...
      card.querySelectorAll('[data-interaction-field]').forEach(input=>input.addEventListener('change',()=>patchInteraction(id,{[input.dataset.interactionField]:input.value||null})));
