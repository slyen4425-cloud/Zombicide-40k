...
assert.deepEqual(plain(sandbox.GensEquipmentAbilityRuntime167885.grantedAbilityIds('h')),['shadow_step','iron_skin','capture_only'],'equipped items should expose their stored refs once each');
const granted=sandbox.GensEquipmentAbilityRuntime167885.grantedAbilities('h');
assert.deepEqual(plain(granted.map(x=>x.id)),['shadow_step','iron_skin'],'only RPG-library abilities should be granted');
assert.equal(granted[0].grantedByEquipment,true);
assert.equal(granted[0].cost,0);
assert.deepEqual(plain(sandbox.dungeonUnlockedActiveTalents('h').map(x=>x.id)),['base_active','shadow_step'],'equipped active ability should join the existing active talent list');
assert.deepEqual(plain(sandbox.dungeonTreeNodesForHero('h').map(x=>x.id)),['base_node','shadow_step','iron_skin'],'equipment abilities should join the existing talent tree result without duplicates');
assert.deepEqual(plain(sandbox.dungeonUnlockedSkillEffectsForHero('h')),[{kind:'base',value:1},{kind:'armor',value:2}],'equipped passive ability effects should reuse the existing passive effect pipeline');
state={inventory:[{itemId:'sword'},{itemId:'armor'}],rightHand:null,leftHand:null,equipment:null,rpgGear:{}};
assert.deepEqual(plain(sandbox.dungeonUnlockedActiveTalents('h').map(x=>x.id)),['base_active'],'unequipping the item must immediately remove the granted ability');
