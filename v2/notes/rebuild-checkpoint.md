...
Nom canonique dans l’overlay d’inspection des créatures possédées :
- `v2/src/modes/capture/creature-inspection.js` utilise maintenant `resolveCaptureSpeciesAsset()` ;
- `buildCaptureCreatureInspection(creature,{speciesDef,assetRegistry})` résout l’ID canonique et le `displayName` depuis le registre Capture ;
- les anciens alias comme `crea_dracendre` deviennent `capture_descendre` / `Descendre` dans l’inspection ;
- `speciesDef` reste prioritaire lorsqu’une vraie définition d’espèce est fournie ;
- sans registre, le comportement historique reste compatible et n’invente aucun nom ;
- aucune image n’a été ajoutée dans l’overlay à cette étape ;
- `capture-page.js` transmet maintenant le même `assetRegistry` à `buildCaptureCreatureInspection()` ;
- aucune donnée source, aucun roster ni aucun état de combat n’est muté.

Régression :
- `v2/tests/capture-creature-inspection.test.mjs` vérifie Aquafin depuis l’ID canonique, la migration visuelle de `crea_dracendre` vers Descendre, la priorité de `speciesDef`, la non-mutation et l’absence de fallback RPG/Dungeon ;
- batterie complète : `34694675567` success.

Commits de l’étape :
- inspection canonique : `fbaddaafacfaa9a9e27bdebb95d04dee49c97d13`
- page / registre transmis à l’inspection : `d955e13fe85b3003829a763fe8c305ef542abebc`
- régression : `14efd58fcb7df398d70b12f12531f1d51b840277`

## Priorités ouvertes

1. prochaine étape Capture : poursuivre la transformation de la page Capture en interface réellement jouable, en exposant progressivement les commandes autoritaires déjà existantes sans réécrire le moteur ;
