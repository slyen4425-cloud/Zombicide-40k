...
import { queueSkillAudio,queueTrapAudio,queueCreatureAudio,queueHeroFormAudio,queueRoomAudio,queueRoomTransitionAudio,queueCombatAudio } from '../src/modes/rpg/audio-runtime.js';
...
const oldAmbienceRequest=out.request;
let started=startAudio(state,oldAmbienceRequest);
assert.equal(started.ok,true); state=started.state;
assert.equal(Object.values(state.playing).some(x=>x.audioId==='crypt-loop'),true);
...
assert.equal(transition.stoppedAmbience.length,1);
assert.equal(transition.stoppedAmbience[0].audioId,'crypt-loop');
assert.equal(Object.values(transition.state.playing).some(x=>x.channel==='ambience'),false);
assert.deepEqual(transition.queued.map(x=>x.phase),['leave','enter','ambience']);
assert.equal(transition.queued[2].audioId,'hall-loop');
