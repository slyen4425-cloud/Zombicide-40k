...
(function(root,factory){
  ...
  function paintDiceOverlay(rt=R){const D=doc(rt),card=D?.querySelector?.(".gtv2DiceCard");if(!card||card.querySelector?.(".gtv2111DiceRow"))return false;const b=currentBattle(rt),row=[...arr(b?.log)].reverse().find(x=>x?.type==="attack"&&num(x?.dice,1)>1&&arr(x?.rollsDisplay).length>1);if(!row)return false;const die=card.querySelector?.("[data-die]");if(!die)return false;const box=D.createElement("div");box.className="gtv2111DiceRow";box.innerHTML=arr(row.rollsDisplay).map(v=>`<div class="gtv2Die">${Math.round(num(v,1))}</div>`).join("");die.replaceWith?.(box);const result=card.querySelector?.("[data-result]");if(result)result.textContent=row.hits?`TOUCHÉ · ${row.hits}/${row.dice} dé(s) · ${row.damage} dégât(s)`:"RATÉ · 0/"+row.dice+" dé(s)";return true}
  ...
  function maintain(rt=R){maintenanceQueued=false;paintWalls(rt);hideRuntimeTabs(rt);ensureDock(rt);paintDiceOverlay(rt);return true}
  ...
  function install(rt=R){ensureStyle(rt);hookAdapter(rt);hookMultiDice(rt);hookDetection(rt);bindClicks(rt);observe(rt);const b=currentBattle(rt);if(b)refreshBattleAttacks(rt,b);maintain(rt);try{rt.GENS_RPG_TACTICAL_RUNTIME_FIXES_VERSION=APP_VERSION}catch(e){}installed=!!(adapterHooked&&rulesHooked);return installed}
