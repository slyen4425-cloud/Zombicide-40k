...
(async()=>{
  ...
  try{
    ...
    assert.equal(first.stubScripts,26,'all 26 Pages runtime script slots must execute before preview-ready');
    ...
    assert.equal(second.stubScripts,26,'runtime composition must execute exactly once after restart');
