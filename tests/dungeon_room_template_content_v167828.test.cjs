...
const context={console,Math,Date,document,setTimeout(fn){fn();return 1},MutationObserver:function(){throw new Error('global MutationObserver interdit dans le template UI')},PointerEvent:function(){},showToast(){},DungeonRoomCreator100:roomApi,DungeonZoneContent167824:zoneApi,DungeonRoomVisualConfig167826:visualApi,DungeonWorldBuilder167821:builderApi};context.window=context;context.globalThis=context;
vm.createContext(context);vm.runInContext(src,context,{filename:'dungeon-room-template-content-167828.js'});
const api=context.DungeonRoomTemplateContent167828;assert.ok(api);assert.equal(api.APP_VERSION,'16.78.29');
...
assert.match(src,/ensureSavedRoom/);assert.match(src,/TEMPLATE_DUNGEON_ID="__room_template__"/);assert.match(src,/⚙️ Variante par zone/);assert.doesNotMatch(src,/new\s+MutationObserver/,'le contenu direct ne doit plus observer toute l’application');assert.match(src,/setTimeout\(refreshEditor,0\)/,'les rafraîchissements doivent suivre uniquement les actions du Créateur');
assert.match(loader,/dungeon-room-template-content-167828\.js\?v=167829/);assert.match(sw,/gensrpg-cache-16\.78\.29-ui-responsiveness/);assert.match(sw,/dungeon-room-template-content-167828\.js/);
const htmlArg=process.argv[2];if(htmlArg){const site=path.dirname(path.resolve(htmlArg));assert.ok(fs.existsSync(path.join(site,'assets','dungeon','dungeon-room-template-content-167828.js')),'le module doit être présent dans le site final');const built=fs.readFileSync(path.join(site,'assets','dungeon','dungeon-room-creator-feedback-167821.js'),'utf8');assert.match(built,/dungeon-room-template-content-167828\.js\?v=167829/);assert.doesNotMatch(built,/new\s+MutationObserver/)}
console.log('Dungeon Room template content V16.78.29 event-driven configuration: OK');
