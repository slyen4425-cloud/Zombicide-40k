...
assert.match(src,/api\(\)\?\.value\?\.\(String\(R\.current\|\|""\),d\.id\)/,'in-game sheet must continue to consume the canonical computed value');
...
const body=fn[1];
assert.doesNotMatch(body,/Base '\+v|Base \$\{?v/,'in-game sheet must not label the computed total as “Base”');
