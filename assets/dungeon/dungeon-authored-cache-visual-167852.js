/* GenSrpG Dungeon — authored cache/runtime visual bridge 16.78.70.
   ...
   uses the validated wall artwork, and normalizes authored enemy token framing.
...
const VERSION="1.3.0",APP_VERSION="16.78.70";
...
function paintBoardVisuals(x,grid,cells){
  ...
  for(const idx of enemyCellIndexes(x)){
    ...
    for(const img of images){const token=enemyTokenRoot(cell,img);token.classList?.add("dav167870EnemyToken");if(token===img)token.classList?.add("dav167870EnemyImage")}
  }
...
ROOT.DungeonAuthoredCacheVisual167852={VERSION,APP_VERSION,FLOOR_THEMES,WALL_ASSET,floorAsset,authoredRoom,enemyTokenRoot,cacheCells,enemyCellIndexes,cleanupLegacyChestScenes,paintBoardVisuals,paint,sync,install};
