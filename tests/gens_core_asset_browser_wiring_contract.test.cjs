...
for(const p of [assetPath,enginePath,adapterPath,uiPath])assert.ok(bootstrap.includes(p),`bootstrap must load ${p}`);
...
assert.equal((bootstrap.match(/assets\/gensrpg\/core\/asset-resolver\.js/g)||[]).length,1,'Core Assets must have one bootstrap load path');
