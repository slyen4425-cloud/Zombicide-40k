...
assert.match(src,/z40kOpenOnline/);assert.match(loader,/gens-multiplayer-entry-167831\.js\?v=167832/);assert.match(sw,/gens-multiplayer-entry-167831\.js/);
const htmlArg=process.argv[2];if(htmlArg){const site=path.dirname(path.resolve(htmlArg));assert.ok(fs.existsSync(path.join(site,'assets','gensrpg','gens-multiplayer-entry-167831.js')),'le bridge multijoueur doit être présent dans le site final');const built=fs.readFileSync(path.join(site,'assets','dungeon','dungeon-room-creator-feedback-167821.js'),'utf8');assert.match(built,/gens-multiplayer-entry-167831\.js\?v=167832/)}
console.log('GenSrpG multiplayer entry retained in V16.78.32: OK');
