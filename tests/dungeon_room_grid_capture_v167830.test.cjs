...
const listeners={pointerCapture:null,pointerBubble:null,pointerUpCapture:null};
const grid={addEventListener(type,fn,capture){if(type==='pointerdown')listeners[capture?'pointerCapture':'pointerBubble']=fn;if(type==='pointerup'&&capture)listeners.pointerUpCapture=fn},removeEventListener(){}};
...
const api=context.DungeonRoomGridCapture167830;assert.ok(api);assert.equal(api.APP_VERSION,'16.78.30');assert.equal(typeof listeners.pointerCapture,'function','pointer capture listener must be attached before historical paint listener');assert.equal(typeof listeners.pointerUpCapture,'function','configuration activation must be deferred to pointerup');
...
function pointerUp(){let stopped=false;const ev={target:cell,preventDefault(){},stopPropagation(){stopped=true},stopImmediatePropagation(){stopped=true}};listeners.pointerUpCapture(ev)}
pointerDown();assert.equal(opened,0,'pointerdown must only block paint and must not open the modal before touch release');assert.equal(painted,0,'historical paint listener must never run while template configuration is active');pointerUp();assert.equal(opened,1,'completed template tap must open editor on pointerup');
templateToggle.classList.set(false);zoneToggle.classList.set(true);pointerDown();assert.equal(zoneOpened,0);assert.equal(painted,0,'historical paint listener must never run while per-zone configuration is active');pointerUp();assert.equal(zoneOpened,1,'completed per-zone tap must activate exact cell');
