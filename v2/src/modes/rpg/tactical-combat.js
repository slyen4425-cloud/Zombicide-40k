import { shortestPathDistance, getActorPosition, moveActor, actorMovementAllowance } from './spatial-engine.js';
...
export function combatDistance(spatial,actorId,targetId,{diagonal=false,maxDistance=999}={}){
...
export function enemiesInContact(spatial,actorId,combat,{diagonal=false}={}){
  const actor=combat?.actors?.[String(actorId)];
  if(!actor) return [];
  return Object.values(combat?.actors||{}).filter(other=>other&&!other.ko&&String(other.id)!==String(actorId)&&String(other.side)!==String(actor.side)&&combatDistance(spatial,actorId,other.id,{diagonal,maxDistance:1})<=1).map(x=>String(x.id));
}

export function evaluateAttackPosition({spatial,combat,actorId,targetId,source,config={}}={}){
  const c=normalizeTacticalCombatConfig(config);
  if(c.mode==='narrative'||!c.rangeEnabled) return {ok:true,distance:null,modifier:0,profile:attackProfile(source,c),contactEnemyIds:[]};
  const profile=attackProfile(source,c);
  const distance=combatDistance(spatial,actorId,targetId,{diagonal:Boolean(config.diagonal),maxDistance:profile.rangeMax});
  if(!Number.isFinite(distance)) return {ok:false,reason:'target-unreachable',distance,modifier:0,profile,contactEnemyIds:[]};
  if(distance<profile.rangeMin) return {ok:false,reason:'too-close',distance,modifier:0,profile,contactEnemyIds:[]};
  if(distance>profile.rangeMax) return {ok:false,reason:'out-of-range',distance,modifier:0,profile,contactEnemyIds:[]};
  const contactEnemyIds=c.contactPenaltyEnabled&&profile.style==='ranged'?enemiesInContact(spatial,actorId,combat,{diagonal:Boolean(config.diagonal)}):[];
  const modifier=contactEnemyIds.length&&!profile.ignoresContactPenalty?profile.contactPenalty:0;
  return {ok:true,distance,modifier,profile,contactEnemyIds};
}
...
export function moveCombatActor({spatial,combat,actorId,target,moveState,config={}}={}){
  ...
  const actor=combat?.actors?.[String(actorId)]||{};
  const moved=moveActor(spatial,actorId,target,{...config,actor,defaultMovement:allowance,movementStatId:null});
