...
assert.equal(pageTags.length,28,'Pages-equivalent characterization expects 28 injected modules including connected Core Stats and Core Inventory Equipment');
...
(async()=>{
  ...
  try{
    ...
    const composition=await page.evaluate(()=>({
      ...
      pagesInjectedModules:28
    }));
    console.log('[phase2-full-capture] composition',JSON.stringify(composition));
    assert.equal(composition.pagesInjectedModules,28,'full production characterization must use the Pages module set');
