...
function el(id=''){return {id,value:'',disabled:false,title:'',dataset:{},innerHTML:'',textContent:'',classList:classList(),style:{setProperty(k,v){this[k]=v}},attrs:{},setAttribute(k,v){this.attrs[k]=String(v)},querySelector(){return null},closest(){return null}}}
...
const api=context.DungeonRoomVisualHotfix167827;assert.ok(api);assert.equal(api.APP_VERSION,'16.78.66');assert.equal(api.WALL_ASSET,'assets/dungeon/creatures/dng_wall_block.jpg');assert.equal(api.autoSelectVisibleContext(),true);assert.equal(changed,1);assert.equal(api.autoSelectVisibleContext(),true);assert.equal(changed,1);api.repairButton();assert.equal(btn.disabled,false);api.retireLegacyUi();assert.equal(legacy.style.display,'none');assert.equal(randomPanel.style.display,'none');
...
assert.match(loader,/dungeon-room-visual-hotfix-167827\.js\?v=167833/);assert.match(loader,/dungeon-room-visual-config-167826\.js\?v=167833/);assert.match(loader,/dungeon-room-grid-capture-167830\.js\?v=167833/);assert.match(loader,/dungeon-room-content-ui-167831\.js\?v=167833/);assert.match(sw,/gensrpg-cache-16\.78\.40-authored-movement/);assert.match(sw,/gensrpg-cache-16\.78\.74-ui-cleanup/);assert.match(sw,/dungeon-room-visual-config-167826\.js/);assert.match(sw,/dungeon-room-visual-hotfix-167827\.js/);assert.match(sw,/dungeon-authored-cache-visual-167852\.js/);assert.match(sw,/dungeon-ui-cleanup-167874\.js/);assert.match(sw,/dungeon-room-grid-capture-167830\.js/);assert.match(sw,/dungeon-room-content-ui-167831\.js/);assert.match(sw,/dng_wall_block\.jpg/);
const htmlArg=process.argv[2];if(htmlArg){const site=path.dirname(path.resolve(htmlArg));assert.ok(fs.existsSync(path.join(site,'assets','dungeon','dungeon-room-visual-hotfix-167827.js')));assert.ok(fs.existsSync(path.join(site,'assets','dungeon','dungeon-room-grid-capture-167830.js')));assert.ok(fs.existsSync(path.join(site,'assets','dungeon','dungeon-room-content-ui-167831.js')));const builtWall=path.join(site,'assets','dungeon','creatures','dng_wall_block.jpg');assert.ok(fs.existsSync(builtWall));assert.ok(fs.statSync(builtWall).size>5000,'built wall asset must not be the old tiny placeholder');const builtLoader=fs.readFileSync(path.join(site,'assets','dungeon','dungeon-room-creator-feedback-167821.js'),'utf8');assert.match(builtLoader,/dungeon-room-visual-hotfix-167827\.js\?v=167833/);assert.match(builtLoader,/dungeon-room-grid-capture-167830\.js\?v=167833/);assert.match(builtLoader,/dungeon-room-content-ui-167831\.js\?v=167833/);assert.doesNotMatch(builtLoader,/new\s+MutationObserver/)}
console.log('Dungeon Room visual hotfix preserved under V16.78.74 UI cleanup: OK');
require('./dungeon_room_visual_theme_v167865.test.cjs');
require('./dungeon_wall_visual_v167866.test.cjs');
