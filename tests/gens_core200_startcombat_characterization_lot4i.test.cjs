...
assert.equal((core200.match(/\bstartCombat\b/g)||[]).length,2,
  'Runtime 2.00 startCombat debt must be exactly definition + dc200 alias after lot 4M');
...
assert.doesNotMatch(core200,/b\.onclick\s*=\s*\(\)=>startCombat\(\[String\(target\.id\)\],'cell'\)/,
  'Lot 4M must remove the final Runtime 2.00 cell lexical startCombat callsite');
assert.match(core200,/b\.onclick\s*=\s*\(\)=>startCellCombat\(target,x\)/,
  'Lot 4M cell action must delegate to the Dungeon-owned preparation helper');
assert.match(core200,/entry:'dc200CellAction'/,
  'Lot 4M cell helper must enter through the canonical Tactical Bridge');
...
assert.equal((core200.match(/entry:'dc200CellAction'/g)||[]).length,1,
  'Runtime 2.00 must expose exactly one canonical cell Bridge entry after lot 4M');
...
console.log('GenSrpG combat lots 4I/4K/4L/4M OK: all active Runtime 2.00 combat actions are Bridge-backed; startCombat remains definition + dc200 alias only');
