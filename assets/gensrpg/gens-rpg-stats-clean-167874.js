...
const CORE=[
 ...
 ["defense","Défense","🛡️",10,0,999,"Réduit la chance adverse de toucher."],
 ["armor","Armure","🧱",0,0,999,"Réduit les dégâts physiques reçus."],
 ["movement","Mouvement","👣",3,0,99,"Nombre de cases disponibles à chaque activation."]
].map(x=>({id:x[0],name:x[1],icon:x[2],defaultValue:x[3],min:x[4],max:x[5],visible:true,description:x[6]}));
const CORE_IDS=new Set(CORE.map(x=>x.id)),SPECIAL_NATIVE=new Set(["defense","armor","movement"]);
const TARGETS=[
 ...
 ["defense","🛡️ Défense"],["armor","🧱 Armure"],["movement","👣 Mouvement"],["initiative","⚡ Initiative"],["enemy_vision","👁️ Champ de vision des ennemis"]
];
const LEGACY=[
...
function root(p=profile()){
 ...
 if(!s[NATIVE_MIGRATION_KEY]){for(const id of SPECIAL_NATIVE)if(!s.active.includes(id))s.active.push(id);s[NATIVE_MIGRATION_KEY]=true}
...
function defs(){return root()?.dynamicDefinitions||[]} function def(id){id=canon(id);return defs().find(x=>x.id===id)||null}
function active(id){return (root()?.active||[]).includes(canon(id))} function effects(){return root()?.[EFFECT_KEY]||[]}
function legacyEffects(){const s=root();if(s?.[LEGACY_MIGRATION_KEY])return [];const r=R.loadDungeonRpgRules?.()||{};return LEGACY.map(x=>({id:x[0],source:x[1],target:x[2],mode:"step",step:Math.max(1,num(r[x[3]],10)),gain:num(r[x[4]],0),threshold:0,comparator:"gt",enabled:true,legacy:true,stepKey:x[3],gainKey:x[4]}))}
function migrateLegacyEffects(p=profile()){const s=root(p);if(!s||s[LEGACY_MIGRATION_KEY])return false;const rules={...(R.loadDungeonRpgRules?.()||{})},current=(Array.isArray(s[EFFECT_KEY])?s[EFFECT_KEY]:[]).map(normEffect).filter(Boolean);for(const x of LEGACY){const [legacyId,source,target,stepKey,gainKey]=x,step=Math.max(1,num(rules[stepKey],10)),gain=num(rules[gainKey],0),replacement=current.some(e=>e.source===source&&e.target===target);if(gain!==0&&!replacement){const id="migrated_"+legacyId;if(!current.some(e=>e.id===id))current.push(normEffect({id,source,target,mode:"step",step,gain,enabled:true}))}rules[gainKey]=0}s[EFFECT_KEY]=current;s.dynamicRules=current.filter(e=>e.mode==="step"&&!String(e.target).startsWith("stat:")).map(e=>({id:e.id,source:e.source,target:e.target,step:e.step,gain:e.gain,enabled:e.enabled}));s[LEGACY_MIGRATION_KEY]=true;try{R.saveDungeonRpgRules?.(rules)}catch(e){}saveProfile(p);return true}
...
function heroDef(hero){try{return R.CHARS?.[hero]||R.findCustomHero?.(hero)||null}catch(e){return null}}
function specialRaw(hero,id){const d=def(id),st=stFor(hero),rec=heroDef(hero),fallback=id==="movement"?num(profile()?.rpgUniverse?.movement?.defaults?.hero,d?.defaultValue??3):num(d?.defaultValue,0);if(Number.isFinite(Number(st?.rpgAttributes?.[id])))return clamp(num(st.rpgAttributes[id]),d?.min??0,d?.max??999);return clamp(num(rec?.dungeonStats?.[id]??rec?.[id],fallback),d?.min??0,d?.max??999)}
...
function syncHeroDefaults(){let changed=false;const custom=defs().filter(d=>!CORE_IDS.has(d.id));try{for(const c of Object.values(R.CHARS||{})){if(!c?.dungeonStats)continue;for(const d of custom)if(c.dungeonStats[d.id]===undefined)c.dungeonStats[d.id]=d.defaultValue}}catch(e){}const hero=String(R.current||""),st=stFor(hero);if(!hero||!st)return false;st.rpgAttributes=st.rpgAttributes&&typeof st.rpgAttributes==="object"?st.rpgAttributes:{};for(const d of runtimeDefs()){if(SPECIAL_NATIVE.has(d.id)&&!Number.isFinite(Number(st.rpgAttributes[d.id]))){st.rpgAttributes[d.id]=specialRaw(hero,d.id);changed=true}else if(!CORE_IDS.has(d.id)&&!Number.isFinite(Number(st.rpgAttributes[d.id]))){st.rpgAttributes[d.id]=num(heroDef(hero)?.dungeonStats?.[d.id],d.defaultValue);changed=true}}if(changed)saveState(hero,st);return changed}
function syncCanonicalRuntime(){syncCanonicalAttributes();return syncHeroDefaults()}
function decorateCanonicalSheet(){if(!D||!dungeon())return false;const host=D.getElementById("dungeonAttributeGrid");if(!host)return false;for(const box of host.querySelectorAll(".dungeonStatBox")){const id=canon(box.querySelector("[data-attr]")?.dataset?.attr||""),d=def(id);if(!d)continue;const strong=box.querySelector("strong");if(strong)strong.textContent=(d.icon?d.icon+" ":"")+d.name;const span=box.querySelector("span");if(span&&SPECIAL_NATIVE.has(id))span.textContent=String(value(String(R.current||""),id));const controls=box.querySelector(".controls"),oldSmalls=[...box.querySelectorAll("small")].filter(x=>!x.hasAttribute("data-clean-stat-explanation"));if(oldSmalls.length>1)oldSmalls[oldSmalls.length-1].style.display="none";let note=box.querySelector("[data-clean-stat-explanation]");if(!note){note=D.createElement("small");note.setAttribute("data-clean-stat-explanation","1");note.style.cssText="display:block;color:#d6c18a;margin:5px 0;line-height:1.35;text-align:left";box.insertBefore(note,controls||null)}const parts=[];if(d.description)parts.push(d.description);const s=summaryFor(id);if(s&&s!=="Aucun effet configuré pour cette stat.")parts.push(s);note.textContent=parts.join(" · ")||"Aucun effet configuré pour cette stat."}return true}
...
function changeSpecial(id,delta){const d=def(id),hero=String(R.current||""),st=stFor(hero);if(!d||!hero||!st||!active(id))return false;st.rpgAttributes=st.rpgAttributes&&typeof st.rpgAttributes==="object"?st.rpgAttributes:{};const prog=R.dungeonActiveProgressionConfig?.()||{},free=prog.attributeEditMode==="free";try{R.dungeonSyncProgressionForState?.(hero,st)}catch(e){}const base=num(heroDef(hero)?.dungeonStats?.[id],d.defaultValue),cur=specialRaw(hero,id),step=num(delta,0);if(step>0&&!free){if(Math.max(0,num(st.statPoints,0))<1){R.alert?.("Aucun point de caractéristique disponible.");return false}st.rpgStatSpent=Math.max(0,num(st.rpgStatSpent,0))+1}if(step<0&&!free){if(cur<=base)return false;st.rpgStatSpent=Math.max(0,num(st.rpgStatSpent,0))-1}st.rpgAttributes[id]=clamp(cur+step,free?d.min:Math.max(d.min,base),d.max);try{R.dungeonSyncProgressionForState?.(hero,st)}catch(e){}saveState(hero,st);try{R.renderDungeonAttributes?.();R.renderDungeonHeroStats?.()}catch(e){}return true}
...
function renderEditor(){if(!D)return false;const host=D.getElementById("rpgStatsList"),p=profile(),s=root(p);if(!host||!p||!s)return false;host.innerHTML='<div class="v2MechanicBox"><div class="v2LibHead"><strong>📊 Caractéristiques & effets</strong><button type="button" data-add-stat>＋ Nouvelle stat</button></div><div class="small">Une stat cochée est disponible en jeu et dans l’éditeur de héros. Les caractéristiques natives utilisent toujours les valeurs du moteur.</div><div data-stat-cards>'+defs().map(cardHtml).join("")+'</div><button class="startGameBtn" type="button" data-save-all>💾 Enregistrer les caractéristiques</button></div>';bindEditor(host);updateLiveSummaries(host);return true}
function bindEditor(host){if(host.dataset.clean795)return;host.dataset.clean795="1";host.addEventListener("click",e=>{const add=e.target.closest("[data-add-stat]");if(add){e.preventDefault();addStat(host);return}const ae=e.target.closest("[data-add-effect]");if(ae){e.preventDefault();addEffect(host,ae.dataset.addEffect);return}const rs=e.target.closest("[data-remove-stat]");if(rs){e.preventDefault();removeStat(host,rs.dataset.removeStat);return}const re=e.target.closest("[data-remove-effect]");if(re){e.preventDefault();removeEffect(host,re.dataset.removeEffect);return}if(e.target.closest("[data-save-all]")){e.preventDefault();const ok=syncEditor(host);renderEditor();renderHeroEditorStats(heroEditorId);try{R.renderDungeonAttributes?.()}catch(_){}R.showToast?.(ok?"💾 Caractéristiques enregistrées":"⚠️ Sauvegarde impossible")}});const live=()=>updateLiveSummaries(host);host.addEventListener("input",live);host.addEventListener("change",live)}
function wrap(name,maker){const old=R[name];if(typeof old!=="function"||old.__clean795)return null;const w=maker(old);w.__clean795=true;w.__original=old;R[name]=w;return old}
function statSnapshot(p=profile()){const s=root(p);return s?{dynamicDefinitions:clone(s.dynamicDefinitions),active:clone(s.active),fx:clone(s[EFFECT_KEY]),dynamicRules:clone(s.dynamicRules),legacy:s[LEGACY_MIGRATION_KEY],native:s[NATIVE_MIGRATION_KEY]}:null}
function restoreSnapshot(snap,p=profile()){if(!snap||!p)return false;const s=root(p);s.dynamicDefinitions=clone(snap.dynamicDefinitions);s.active=clone(snap.active);s[EFFECT_KEY]=clone(snap.fx);s.dynamicRules=clone(snap.dynamicRules);s[LEGACY_MIGRATION_KEY]=snap.legacy;s[NATIVE_MIGRATION_KEY]=snap.native;return saveProfile(p)}
function renderHeroEditorStats(id=heroEditorId){if(!D)return false;heroEditorId=id??heroEditorId;const host=D.getElementById("hcRpgStatsFields");if(!host)return false;const activeIds=new Set(runtimeDefs().map(d=>d.id));for(const [sid,inputId] of Object.entries(HERO_INPUTS)){const input=D.getElementById(inputId),label=input?.closest?.("label");if(!label)continue;const d=def(sid),on=activeIds.has(sid);label.style.display=on?"":"none";if(on&&d){const first=[...label.childNodes].find(n=>n.nodeType===3);if(first)first.textContent=(d.icon?d.icon+" ":"")+d.name+" "}}for(const old of host.querySelectorAll("[data-gens-dynamic-hero-stat-label]"))old.remove();const rec=heroDef(heroEditorId);for(const d of runtimeDefs()){if(HERO_INPUTS[d.id])continue;const label=D.createElement("label");label.setAttribute("data-gens-dynamic-hero-stat-label",d.id);label.textContent=(d.icon?d.icon+" ":"")+d.name+" ";const input=D.createElement("input");input.type="number";input.min=String(d.min);input.max=String(d.max);input.value=String(num(rec?.dungeonStats?.[d.id],d.defaultValue));input.setAttribute("data-gens-dynamic-hero-stat",d.id);label.appendChild(input);const elements=D.getElementById("hcRpgElementsWrap");host.insertBefore(label,elements||null)}return true}
...
function installRuntime(){
 ...
 wrap("renderRpgUniverseEditor",old=>function(){const out=old.apply(this,arguments);setTimeout(()=>{renderEditor();renderHeroEditorStats(heroEditorId)},0);return out});
 wrap("saveRpgUniverseStats",old=>function(){try{const h=D?.getElementById("rpgStatsList");if(h)syncEditor(h)}catch(e){}const snap=statSnapshot(),out=old.apply(this,arguments);restoreSnapshot(snap);setTimeout(()=>{renderEditor();syncCanonicalRuntime();renderHeroEditorStats(heroEditorId)},0);return out});
 ...
 wrap("dungeonDerivedDefense",old=>function(){return dungeon()&&def("defense")?(active("defense")?Math.max(0,value(String(R.current||""),"defense")):0):old.apply(this,arguments)});
 wrap("dungeonArmorScore",old=>function(){return dungeon()&&def("armor")?(active("armor")?Math.max(0,value(String(R.current||""),"armor")):0):old.apply(this,arguments)});
 wrap("dungeonDerivedInitiative",old=>function(){return num(old.apply(this,arguments),0)+extraTotal("initiative")});
 wrap("dungeonHeroMoveValue083",old=>function(heroId){if(!dungeon()||!def("movement"))return old.apply(this,arguments);return active("movement")?Math.max(0,value(String(heroId||R.current||""),"movement")):movementFallback()});
 ...
 wrap("openHeroCreator",old=>function(id){heroEditorId=id??null;const out=old.apply(this,arguments);setTimeout(()=>renderHeroEditorStats(id),0);return out});
 wrap("hcRenderRpgStatsUsage",old=>function(){const out=old.apply(this,arguments);setTimeout(()=>renderHeroEditorStats(heroEditorId),0);return out});
