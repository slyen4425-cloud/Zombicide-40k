...
assert.equal(total,4,'Phase 5 target must reduce startConfiguredGame assignments from 5 to 4');
assert.deepEqual(ids,[
  'captureFix138',
  'captureFix139',
  'gensDungeonCore01Js',
  'dungeonCore200Rebuild'
],'Phase 5 captureFix135 retirement target chain drifted');

const capture135=blocks.find(x=>x.id==='captureFix135');
assert.ok(capture135,'captureFix135 block must remain present');
assert.doesNotMatch(capture135.body,/window\.startConfiguredGame\s*=/,
  'captureFix135 must no longer assign startConfiguredGame');
