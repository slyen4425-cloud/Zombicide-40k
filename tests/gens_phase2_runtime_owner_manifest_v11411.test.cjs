...
assert.equal(reachable.size,68,'production graph size drifted before owner validation');
assert.equal(mapped.size,68,'owner manifest must currently map all 67 production-reachable files');
