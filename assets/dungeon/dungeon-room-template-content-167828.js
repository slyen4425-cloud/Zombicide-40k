...
const VERSION="1.3.0",APP_VERSION="16.78.44",TEMPLATE_DUNGEON_ID="__room_template__";
...
function editorHasRoom(){const e=DOC?.getElementById("drc100Editor"),g=DOC?.getElementById("drc100Grid");return !!g&&(!e||e.style?.display!=="none")}
function inferRoomId(){
  const api=roomApi();if(!api||!DOC)return "";
  const name=String(DOC.getElementById("drc100Name")?.value||"").trim(),w=Number(DOC.getElementById("drc100Width")?.value),h=Number(DOC.getElementById("drc100Height")?.value);
...
function configureTemplate(roomId,cell,obj,data){const id=String(roomId||inferRoomId()),api=visualApi();if(!id||!api?.configureElement)return null;const out=api.configureElement(TEMPLATE_DUNGEON_ID,id,id,Number(cell),String(obj),data||{});propagateToInheritedZones(id);return out}
function removeTemplate(roomId,cell,obj){const id=String(roomId||inferRoomId()),api=visualApi();if(!id||!api?.removeElement)return null;const out=api.removeElement(TEMPLATE_DUNGEON_ID,id,Number(cell),String(obj));propagateToInheritedZones(id);return out}
...
function decorateGrid(){
  if(!DOC)return;const r=room(),grid=DOC.getElementById("drc100Grid");if(!grid)return;
  grid.querySelectorAll("[data-drc-index]").forEach(el=>{clearOwnHandler(el);el.classList.remove("drt167828Configurable","drt167828Configured")});
  if(!templateMode||!r)return;const content=templateContent(r.id);
  grid.querySelectorAll("[data-drc-index]").forEach(el=>{const i=Number(el.dataset.drcIndex),obj=String(r.cells?.[i]?.object||"");if(CONFIGURABLE.has(obj)){el.classList.add("drt167828Configurable");if(specAt(content,obj,i))el.classList.add("drt167828Configured")}const handler=ev=>{block(ev);if(CONFIGURABLE.has(obj))openEditor(i,obj);else try{ROOT.showToast?.("Touche un ennemi, boss, coffre, piège ou énigme déjà placé.")}catch(e){}};el.__drt167828Handler=handler;el.onpointerdown=handler;if(typeof ROOT.PointerEvent==="undefined")el.onclick=handler});
}
...
function openEditor(cell,obj){
  ...
  if(obj==="enemy"||obj==="boss")body='<label>Monstre<select id="drtEnemy">'+enemyOptions(String(old?.enemyId||"dng_skeleton"))+'</select></label><label>Quantité<input id="drtQty" type="number" min="1" max="50" value="'+Number(old?.qty||1)+'"></label><label style="align-content:end"><span><input id="drtKey" type="checkbox" '+(old?.hasKey?'checked':'')+'> Porte la clé</span></label>';
  ...
  else if(obj==="puzzle")body='<label class="drtFull">Énigme existante<input id="drtPuzzleRef" value="'+esc(old?.refId||'')+'"></label><label>Cible<select id="drtTarget"><option value="cell">Case</option><option value="door">Porte</option><option value="chest">Coffre</option></select></label><label>Élément cible<select id="drtTargetIndex"></select></label><label>Nom / note<input id="drtPuzzleLabel" value="'+esc(old?.label||'')+'"></label>';
