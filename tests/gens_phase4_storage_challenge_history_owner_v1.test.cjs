...
assert.equal(bytes.length,8174346,'post-raccord index size must match deterministic Challenge History micro-diff');
assert.equal(blob,'8da7afa3c986f29e740eee1748dcc0ec0f8f75bc','post-raccord index blob must match deterministic Challenge History micro-diff');
