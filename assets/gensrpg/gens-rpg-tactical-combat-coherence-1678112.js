...
(function(root,factory){
  ...
  function ensureStyle(rt=R){const D=doc(rt);if(!D||D.getElementById?.(STYLE_ID))return !!D;const s=D.createElement("style");s.id=STYLE_ID;s.textContent=`
    ...
    .${WALL_CLASS},.gtv2Cell.blocked,#drc100Grid .drc100Cell.wall{position:relative!important;background-image:none!important;overflow:hidden!important}
  ...
  function markWallCells(rt=R){const D=doc(rt);if(!D)return 0;let n=0;const state=runtimeState(rt),kinds=arr(state?.last?.map?.cells),cells=[...(D.querySelectorAll?.("#dc047RoomBoard .dc047Grid > .dc047Cell")||[])];for(let i=0;i<cells.length;i++){const wall=normalizeText(kinds[i])==="wall"||cells[i].classList?.contains?.("dav167870WallCell");if(!wall)continue;cells[i].classList?.add(WALL_CLASS);cells[i].style?.setProperty?.("background-image","none","important");n++}for(const el of D.querySelectorAll?.(".gtv2Cell.blocked,#drc100Grid .drc100Cell.wall,#dc047RoomBoard .dav167870WallCell")||[]){el.classList?.add(WALL_CLASS);el.style?.setProperty?.("background-image","none","important");n++}return n}
  function patchDice(rt=R){const D=doc(rt),card=D?.querySelector?.(".gtv2DiceCard");if(!card)return false;try{rt?.GensRpgTacticalRuntimeFixes1678111?.paintDiceOverlay?.(rt)}catch(e){}const row=latestAttackRow(rt);if(!row)return false;let note=card.querySelector?.(".gtv2112Explain");if(!note){note=D.createElement("div");note.className="gtv2112Explain";const button=card.querySelector?.("[data-dice-continue]");if(button)card.insertBefore(note,button);else card.appendChild(note)}note.textContent=explainAttack(row);for(const [i,die] of [...card.querySelectorAll?.(".gtv2111DiceRow .gtv2Die,.gtv2Die")||[]].entries()){die.style.animationDelay=(i*70)+"ms"}return true}
  function maintain(rt=R){queued=false;ensureStyle(rt);markWallCells(rt);patchDetail(rt);patchDice(rt);return true}
  ...
  function install(rt=R){ensureStyle(rt);hookAdapter(rt);hookStart(rt);hookResultDetails(rt);hookDetection(rt);bindDetail(rt);observe(rt);maintain(rt);scheduleDetection(rt,"install-vision-v112",true);try{rt.GENSRPG_VERSION=APP_VERSION;rt.GENS_RPG_TACTICAL_COHERENCE_VERSION=APP_VERSION}catch(e){}installed=!!(adapterHooked&&startHooked&&resultHooked);return installed}
