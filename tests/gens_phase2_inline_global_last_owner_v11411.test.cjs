...
assert.equal(blob,'9313afd3437fe827b9c17245a675f75645570878','Phase 2 inline global table must target the current committed index blob');
assert.equal(distinctGlobals,438,'distinct explicit inline globals drifted');
assert.equal(assignments,771,'explicit inline global assignments drifted');
...
for(const [name,count,lastOwner] of [
  ...
  ['startConfiguredGame',4,'dungeonCore200Rebuild'],
