/* GenSrpG Dungeon — authored cache/runtime visual bridge 16.78.73.
...
const VERSION="1.3.3",APP_VERSION="16.78.73";
...
function install(){if(wrap()){for(const ms of [0,50,250])setTimeout(()=>{sync();installResizeSync();installTokenMutationSync()},ms);return true}if(retries++<30&&typeof setTimeout==="function")setTimeout(install,100);return false}
ROOT.DungeonAuthoredCacheVisual167852={VERSION,APP_VERSION,FLOOR_THEMES,WALL_ASSET,ACTOR_RATIO,ACTOR_RATIO_MEDIUM,ACTOR_RATIO_COMPACT,ACTOR_ZOOM,floorAsset,authoredRoom,enemyTokenRoot,isActorImage,actorRatioForSide,actorTokenSize,fitActorToken,refitDc310Tokens,installTokenMutationSync,cacheCells,enemyCellIndexes,heroCellIndexes,actorCellIndexes,cleanupLegacyChestScenes,paintBoardVisuals,paint,sync,install};
