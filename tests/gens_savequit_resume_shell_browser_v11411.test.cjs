...
(async()=>{
  let stage='boot';
  const mark=name=>{stage=name;console.log('[savequit-resume]',name)};
  const watchdog=setTimeout(()=>{console.error('[savequit-resume] WATCHDOG stage='+stage);process.exit(1)},150000);
